document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchAllTechnicians() {
        fetch('/workforce-manager/api/get_technician_mappings')
    ...
    function fetchTechnicianGroups() {
        fetch('/workforce-manager/api/technician_groups') // Corrected endpoint
            .then(response => response.json())
            .then(data => {
                allGroups = data;
    ...
    technicianGroupListContainer.addEventListener('click', function (e) {
        ...
        if (editBtn) {
            ...
            if (confirm('Are you sure you want to delete this group?')) {
                fetch(`/workforce-manager/api/technician_groups/${groupId}`, { // Corrected endpoint
                    method: 'DELETE',
                    headers: {
                        'X-CSRFToken': csrfToken
                    }
    ...
    function loadGroupMembers(groupId) {
        fetch(`/workforce-manager/api/technician_groups/${groupId}/members`) // Corrected endpoint
            .then(response => response.json())
            .then(members => {
    ...
    function moveTechnicians(sourceList, isAdding) {
        ...
        const promises = technicianIds.map(technicianId => {
            return fetch('/workforce-manager/api/technician_groups/members', { // Corrected endpoint
