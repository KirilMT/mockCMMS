...
async function fetchTechnicianSkillUpgradeLogs(technicianId) {
    try {
        const response = await fetch(`/workforce-manager/api/technician_skill_upgrade_logs/${technicianId}`);
...
async function fetchTechnicianSkills(technicianName) {
    ...
    try {
        ...
        const response = await fetch(`/workforce-manager/api/technician_skills/${currentSelectedTechnicianId}`);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
...
async function updateTechnicianSkill(technicianName, technicianId, technologyId, skillLevel) {
    ...
    try {
        const response = await fetch('/workforce-manager/api/technician_skill', {
