...
async function fetchAllTechnologies() {
    try {
        const response = await fetch('/workforce-manager/api/technologies');
...
async function editTechnology(techId) {
    ...
    techListItem.querySelector('.save-tech-btn').addEventListener('click', async () => {
        ...
        if (newName) {
            ...
            try {
                const response = await fetch(`/workforce-manager/api/technologies/${techId}`, {
                    method: 'PUT',
...
async function deleteTechnology(techId) {
    ...
    if (!confirm(`Are you sure you want to delete \"${escapeHtml(techName)}\"? This may affect child technologies, task mappings, and technician skills.`)) { // Removed ID
        return;
    }
    try {
        const response = await fetch(`/workforce-manager/api/technologies/${techId}`, {method: 'DELETE'});
        const result = await response.json();
        if (response.ok) {
...
async function fetchTechnologyGroups() {
    try {
        const response = await fetch('/workforce-manager/api/technology_groups');
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
...
async function editTechnologyGroup(groupId, currentName) {
    ...
    groupListItem.querySelector('.save-group-btn').addEventListener('click', async () => {
        ...
        if (newName && newName !== currentName) {
            try {
                const response = await fetch(`/workforce-manager/api/technology_groups/${groupId}`, {
                    method: 'PUT',
                    headers: {'Content-Type': 'application/json'},
...
async function deleteTechnologyGroup(groupId) {
    ...
    try {
        const response = await fetch(`/workforce-manager/api/technology_groups/${groupId}`, {method: 'DELETE'});
        const result = await response.json();
        if (response.ok) {
