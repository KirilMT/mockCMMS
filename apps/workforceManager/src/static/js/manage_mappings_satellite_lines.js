...
async function loadSatellitePoints() {
    try {
        const response = await window.fetch_get('/workforce-manager/api/satellite_points');
        if (!response.ok) {
...
async function handleAddSatellitePoint() {
    ...
    try {
        const response = await window.fetch_post('/workforce-manager/api/satellite_points', { name });
...
async function executeUpdateSatellitePoint(pointId, newName) { // newName is raw
    try {
        const response = await window.fetch_put(`/workforce-manager/api/satellite_points/${pointId}`, { name: newName });
...
async function handleDeleteSatellitePoint(event) {
    ...
    try {
        const response = await window.fetch_delete(`/workforce-manager/api/satellite_points/${pointId}`);
...
async function loadLines() {
    try {
        const response = await window.fetch_get('/workforce-manager/api/lines');
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Invalid JSON response' }));
...
async function handleAddLine() {
    ...
    try {
        const response = await window.fetch_post('/workforce-manager/api/lines', { name, satellite_point_id: parseInt(satellitePointId) });
        const responseData = await response.json();
...
async function executeUpdateLine(lineId, newName, newSatellitePointId) { // newName is raw
    try {
        const response = await window.fetch_put(`/workforce-manager/api/lines/${lineId}`, { name: newName, satellite_point_id: newSatellitePointId });
        const responseData = await response.json().catch(() => ({ message: 'Invalid JSON response' }));
...
async function handleDeleteLine(event) {
    ...
    try {
        const response = await window.fetch_delete(`/workforce-manager/api/lines/${lineId}`);
...
async function handleTechnicianFormSubmit(event) {
    ...
    try {
        ...
        if (technicianId) {
            ...
            response = await window.fetch_put(`/workforce-manager/api/technicians/${technicianId}`, technicianData);
        } else {
            // Add new technician
            response = await window.fetch_post('/workforce-manager/api/technicians', technicianData);
        }
        const responseData = await response.json().catch(() => ({ message: 'Invalid JSON response' }));
...
async function loadTechnicians() {
    try {
        const response = await window.fetch_get('/workforce-manager/api/technicians');
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Invalid JSON response' }));
...
function handleEditTechnician(event) {
    ...
    fetch(`/workforce-manager/api/technicians/${technicianId}`)
...
async function handleDeleteTechnician(event) {
    ...
    try {
        const response = await window.fetch_delete(`/workforce-manager/api/technicians/${technicianId}`);
