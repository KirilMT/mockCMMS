...
async function fetchAndPopulateSatellitePointsDropdowns() { // Renamed to reflect multiple dropdowns
    try {
        const response = await fetch('/workforce-manager/api/satellite_points');
        if (!response.ok) {
...
async function fetchMappings(technicianNameToSelect = null) {
    try {
        const response = await fetch('/workforce-manager/api/get_technician_mappings');
...
async function handleTechSatellitePointChange() {
    ...
    try {
        const response = await fetch('/workforce-manager/api/save_technician_mappings', {
            method: 'POST',
...
async function handleSaveNewTechnician() {
    ...
    try {
        ...
        const response = await fetch('/workforce-manager/api/technicians', {
            method: 'POST',
            headers: {
...
async function handleEditTechnicianName() {
    ...
    if (newNamePrompt && newNamePrompt.trim() !== '' && newNamePrompt.trim() !== oldName) {
        ...
        try {
            const response = await fetch(`/workforce-manager/api/technicians/${currentSelectedTechnicianId}`,
            {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
