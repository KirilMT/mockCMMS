...
@pytest.fixture(scope="module")
def workforce_manager_server_api_mode():
    """Fixture to run the workforceManager server in API mode."""
    ...
    while retries > 0:
        ...
        raise RuntimeError("workforceManager server did not start in time.")
...
def test_full_integration_workflow(mock_cmms_server, workforce_manager_server_api_mode):
    """
    Tests the full integration workflow:
    1. Logs into mockCMMS.
    2. Navigates to the embedded workforceManager page.
    3. Verifies workforceManager loads and uses data from mockCMMS.
    """
    mock_cmms_url = mock_cmms_server
    workforce_manager_url = workforce_manager_server_api_mode
    ...
    # 2. Navigate to the embedded workforceManager page within mockCMMS
    wm_embed_response = session.get(f"{mock_cmms_url}/workforce-manager")
    assert wm_embed_response.status_code == 200
    assert "Workforce Manager Integration" in wm_embed_response.text
    assert f'<iframe src="{workforce_manager_url}/"' in wm_embed_response.text

    # 3. Verify workforceManager loads and uses data from mockCMMS
    # Since workforceManager is embedded via iframe, we need to make a direct request
    # to workforceManager's dashboard to check its data.
    # The index_route in workforceManager redirects to manage_mappings_route when DATA_SOURCE=api
    
    # Make a request to workforceManager's manage_mappings_route
    # This will trigger the data loading from mockCMMS API
    wm_manage_mappings_response = session.get(f"{workforce_manager_url}/manage_mappings_ui")
    ...
    # or making direct API calls to workforceManager's internal data endpoints if available.
    assert "Manage Mappings" in wm_manage_mappings_response.text
    
    # A more direct way to check if data was loaded would be to call workforceManager's
    ...
    # Example: If workforceManager had an API to list loaded tasks:
    # wm_tasks_api_response = session.get(f"{workforce_manager_url}/api/tasks_loaded")
