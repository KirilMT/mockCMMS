# apps/workforceManager/tests/test_inventory_integration.py

import pytest
from src.services.db_utils import db, MaintenanceOrder, SparePart, maintenance_order_spare_parts
from apps.workforceManager.src.services.inventory_service import check_spare_parts_availability, get_tasks_with_insufficient_parts
...
def app_context_inventory():
    ...
    with app.app_context():
        ...
        mo1 = MaintenanceOrder(id=101, asset_id=1, description="Task with sufficient parts", order_type="PM")
        mo2 = MaintenanceOrder(id=102, asset_id=1, description="Task with insufficient parts", order_type="PM")
        mo3 = MaintenanceOrder(id=103, asset_id=1, description="Task with zero stock parts", order_type="PM")
        mo4 = MaintenanceOrder(id=104, asset_id=1, description="Task with no parts required", order_type="PM")

        db.session.add_all([part1, part2, part3, mo1, mo2, mo3, mo4])
        db.session.commit()

        # Link parts to MOs
        db.session.execute(maintenance_order_spare_parts.insert().values(maintenance_order_id=101, spare_part_id=1, quantity_required=5))
        db.session.execute(maintenance_order_spare_parts.insert().values(maintenance_order_id=102, spare_part_id=2, quantity_required=3))
        db.session.execute(maintenance_order_spare_parts.insert().values(maintenance_order_id=103, spare_part_id=3, quantity_required=1))
        db.session.commit()
...
def test_check_parts_sufficient(app_context_inventory):
    ...
    mo = MaintenanceOrder.query.get(101)
...
def test_check_parts_insufficient(app_context_inventory):
    ...
    mo = MaintenanceOrder.query.get(102)
    is_available, details = check_spare_parts_availability(mo)
...
def test_check_parts_zero_stock(app_context_inventory):
    ...
    mo = MaintenanceOrder.query.get(103)
    is_available, details = check_spare_parts_availability(mo)
...
def test_check_no_parts_required(app_context_inventory):
    ...
    mo = MaintenanceOrder.query.get(104)
    is_available, details = check_spare_parts_availability(mo)
