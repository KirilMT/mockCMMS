...
It merges the routes from main.py, api.py, and health.py into a single Blueprint that
...
def cleanup_expired_sessions():
    ...
    expired_sessions = [
        sid
        for sid, sdata in session_excel_data_cache.items()
        if isinstance(sdata, dict)
        and "timestamp" in sdata
        and current_time - sdata["timestamp"] > SESSION_TIMEOUT_SECONDS
    ]
...
def get_session_data(session_id):
    return (
        session_excel_data_cache[session_id]["data"]
...
def index_route():
    ...
    active_schedule = (
        Schedule.query.filter_by(planning_status="Planned")
    ...
    return render_template(
        "planning/index.html", schedules=schedules, active_schedule=active_schedule
    )
...
def upload_file_route():
    current_app.logger.info(
        f"Upload request received - Content-Type: {request.content_type}"
    )
    try:
        ...
        try:
            session_id = InputValidator.validate_string(
                session_id, max_length=100, pattern=r"^[a-zA-Z0-9\-_]+$"
            )
        except ValueError:
        ...
        if "excelFile" in request.files and request.files["excelFile"].filename != "":
            ...
            try:
                ...
                engine = (
                    "pyxlsb"
                    if getattr(excel_file_stream, "filename", "")
                    .lower()
                    .endswith(".xlsb")
                    else "openpyxl"
                )
                with pd.ExcelFile(BytesIO(excel_file_copy), engine=engine) as xls:
                    if f"Summary KW{current_week_number}" not in xls.sheet_names:
                        weeks = sorted(
                            [
                                s.replace("Summary KW", "")
                                for s in xls.sheet_names
                                if s.startswith("Summary KW")
                            ]
                        )
                        return (
                            jsonify(
                ...
                store_session_data(
                    ...
                    [
                        {"id": str(i + 1), **item}
                        for i, item in enumerate(excel_data_list)
                    ],
                )
                pm_tasks = [
                    {
                        "id": str(i + 1),
                        "name": t.get("scheduler_group_task", "Unknown"),
                        **t,
                    }
                    for i, t in enumerate(
                        sanitize_data(excel_data_list, current_app.logger)
                    )
                    if t.get("task_type", "").upper() == "PM"
                ]
                msg = (
                    f"File processed. {len(errors)} issues found."
                    if errors
                    else "File processed. PM tasks extracted."
                )
                return jsonify(
                    {
                        "message": msg,
                ...
                current_app.logger.error(
                    f"Error during file upload: {e}", exc_info=True
                )
            ...
            try:
                ...
                present = [
                    tech
                    for group in TECHNICIAN_GROUPS.values()
                    for tech in group
                    if tech not in absent
                ]
                total_time = calculate_work_time(get_current_day())
                tasks = sanitize_data(cached_data, current_app.logger)
                rep_tasks = [
                    t for t in tasks if t.get("task_type", "").upper() == "REP"
                ]
                eligible = {
                    t["id"]: [
                        {
                            ...
                            "task_full_duration": int(t.get("planned_worktime_min", 0)),
                        }
                    ]
                    for t in rep_tasks
                    for tech in present
                ...
                return jsonify(
                    {
                        "message": "REP task data prepared.",
                        "rep_tasks": rep_tasks,
                ...
                current_app.logger.error(
                    f"Error processing absent technicians: {e}", exc_info=True
                )
                return jsonify({"message": f"Error: {e}"}), 500
        return jsonify({"message": "Invalid request."}), 400
    except Exception as e:
        current_app.logger.error(
            f"Unexpected error in upload_file_route: {e}", exc_info=True
        )
...
def generate_dashboard_route():
    try:
        ...
        final_tasks = {
            str(t.get("id")): {**t, "name": t.get("name", f"Task {t.get('id')}")}
            for t in ui_tasks
        }
        for task in cached_data:
            ...
            if (
                tid
                and tid not in final_tasks
                and task.get("task_type", "").upper() == "PM"
            ):
                final_tasks[tid] = {
                    **task,
            ...
            task.update(
                {
                    ...
                    "technology_ids": [
                        s["technology_id"]
                        for s in task_manager.get_required_skills(db_id)
                    ],
                }
            )
        g.db.commit()
        time_summary, under_resourced = generate_html_files(
            all_tasks=list(final_tasks.values()),
            present_technicians=present,
            rep_assignments=rep_assign,
            ...
            technician_technology_skills=skills_map,
        )
        dash_url = (
            url_for(
                "planning.output_file_route",
        ...
        return jsonify(
            {
                ...
                "available_time": time_summary,
                "under_resourced_tasks": under_resourced,
                "session_id": session_id,
        ...
        current_app.logger.error(
            f"Error in generate_dashboard_route: {e}", exc_info=True
        )
        if hasattr(g, "db"):
            g.db.rollback()
...
def technicians_api():
    if request.method == "GET":
        try:
            ...
            current_app.logger.error(
                f"Error in GET /api/technicians: {e}", exc_info=True
            )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if cursor.fetchone():
                return jsonify({"message": f"Technician '{name}' already exists."}), 409
            cursor.execute(
            ...
            if g.db:
                g.db.rollback()
            current_app.logger.error(
                f"DB/Value error POST /api/technicians for '{name}': {e}", exc_info=True
            )
            ...
            if g.db:
                g.db.rollback()
            current_app.logger.error(
                f"Generic error POST /api/technicians for '{name}': {e}", exc_info=True
            )
...
def manage_technician_api(technician_id):
    if request.method == "PUT":
        ...
        try:
            ...
            name = data.get("name", "").strip()
            satellite_id = data.get("satellite_point_id")
            if not name and satellite_id is None:
            ...
            if (
                name
                and cursor.execute(
                    "SELECT id FROM technicians WHERE name = ? AND id != ?",
                    (name, technician_id),
                ).fetchone()
            ):
            ...
            if name and satellite_id is not None:
                ...
                cursor.execute(
                    "UPDATE technicians SET satellite_point_id = ? WHERE id = ?",
                    (satellite_id, technician_id),
                )
            g.db.commit()
            cursor.execute(
                "SELECT id, name, satellite_point_id FROM technicians WHERE id = ?",
    ...
    if request.method == "DELETE":
        try:
            ...
            cursor.execute(
                "DELETE FROM technician_task_assignments WHERE technician_id = ?",
                (technician_id,),
            )
...
def add_task_api():
    try:
        ...
        if not task_name or not tech_ids:
            return (
                jsonify({"message": "Task name and technology IDs are required."}),
                400,
            )
        task_manager = TaskManager(g.db)
        if task_manager.get_by_name(task_name):
    ...
    try:
        if request.method == "GET":
            ...
            try:
                cursor = g.db.cursor()
                cursor.execute(
                    "INSERT INTO satellite_points (name) VALUES (?)", (name,)
                )
                g.db.commit()
        ...
        if request.method == "PUT":
            ...
            if name:
                try:
                    cursor = g.db.cursor()
                    cursor.execute(
                        "UPDATE satellite_points SET name = ? WHERE id = ?",
                        (name, point_id),
                    )
                    g.db.commit()
                except Exception:
...
def get_technician_mappings_api():
    try:
        ...
        current_app.logger.error(
            f"Error in get_technician_mappings_api: {e}", exc_info=True
        )
...
def health_check():
    try:
        ...
        return (
            jsonify({"status": "unhealthy", "error": "Health check system failure"}),
            503,
        )
...
def readiness_check():
    try:
        ...
        return (
            jsonify({"status": "ready" if db_ok and config_ok else "not_ready"}),
            200 if db_ok and config_ok else 503,
        )
    except Exception as e:
            ...
            jsonify(
                {
                    ...
                    "environment_variables": {
                        k: v for k, v in os.environ.items() if "SECRET" not in k.lower()
                    },
                }
            ),
            200,
...
def create_schedule():
    ...
    try:
        ...
        # Create schedule
        schedule = Schedule(
            name=name, start_date=start_date, end_date=end_date, planning_status="Draft"
        )
        ...
        for pt in planning_tasks:
            ...
            if mo:
                ...
                if pt.assigned_user:
                    ...
                    assigned_to_skills = (
                        ", ".join([us.skill.name for us in pt.assigned_user.skills])
                        if pt.assigned_user.skills
                        else ""
                    )
                ...
                task_data_json.append(
                    {
                        ...
                        "required_skills": (
                            ", ".join([skill.name for skill in mo.required_skills])
                            if mo.required_skills
                            else "None"
                        ),
...
def run_planning(schedule_id):
    ...
    try:
        ...
        result = engine.generate_plan(
            schedule=schedule, planning_mode=planning_mode, check_parts=check_parts
        )
        ...
        for assignment in result.assigned_tasks:
            ...
            if planning_task:
                ...
                planning_task.actual_duration_minutes = (
    ...
    try:
        ...
        for pt in planning_tasks:
            try:
                ...
                # Get required skills
                required_skills = (
                    [skill.name for skill in mo.required_skills]
                    if hasattr(mo, "required_skills")
                    else []
                )
                ...
                task_data = {
                    ...
                    "planned_start_time": (
                        pt.planned_start_time.isoformat()
                        if pt.planned_start_time
                        else None
                    ),
                    "planned_end_time": (
                        pt.planned_end_time.isoformat() if pt.planned_end_time else None
                    ),
        ...
        while current_date <= schedule.end_date:
            ...
            active_pattern = (
                "Pattern 1" if is_odd_week else "Pattern 2"
...
def publish_schedule(schedule_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "message": f'Schedule "{schedule.name}" published'}
            ),
            200,
        )
