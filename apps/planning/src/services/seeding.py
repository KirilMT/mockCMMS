...
def seed_planning_data(logger=None):
    ...
    try:
        if Schedule.query.first():
            logger.info("Planning database already contains data. Skipping main population.")
        ...
        for sched_info in schedules_data:
            start_date = datetime.strptime(sched_info["start_date"], "%Y-%m-%dT%H:%M:%S")
            ...
            if created:
                ...
                for mo_desc in mo_list:
                    ...
                    mo = db.session.query(MaintenanceOrder).filter_by(description=mo_desc).first()
                    if mo:
                        # Create PlanningTask
                        pt = PlanningTask(
