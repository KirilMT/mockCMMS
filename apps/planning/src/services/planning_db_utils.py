...
def populate_dummy_data(conn, logger):
    ...
    for tech_data in data.get("technologies", []):
        if "parent" in tech_data:
            ...
            tech_id = tech_manager.get_or_create(
                tech_name, group_id=group_id, parent_id=parent_id
            )
            technologies[tech_name] = tech_id
    ...
    logger.info(
        f"Populated {len(tasks)} tasks and their required skills and conditions."
    )
...
def update_satellite_point(conn, point_id, new_name):
    ...
    try:
        cursor.execute(
            "UPDATE satellite_points SET name = ? WHERE id = ?", (new_name, point_id)
        )
        conn.commit()
...
def delete_satellite_point(conn, point_id):
    """Deletes a satellite point if it's not associated with any lines or
    technicians."""
    cursor = conn.cursor()
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM technicians WHERE satellite_point_id = ?", (point_id,)
    )
    if cursor.fetchone()[0] > 0:
        return (
            False,
            "Satellite point is associated with technicians and cannot be deleted.",
        )
...
def add_line(conn, name, satellite_point_id):
    ...
    cursor.execute(
        "SELECT id FROM lines WHERE name = ? AND satellite_point_id = ?",
        (name, satellite_point_id),
    )
    if cursor.fetchone():
...
def get_lines_for_satellite_point(conn, satellite_point_id):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, name FROM lines WHERE satellite_point_id = ? ORDER BY name",
        (satellite_point_id,),
    )
    return [{"id": row["id"], "name": row["name"]} for row in cursor.fetchall()]
...
def update_line(conn, line_id, new_name, new_satellite_point_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id FROM satellite_points WHERE id = ?", (new_satellite_point_id,)
        )
...
class TechnologyManager:
    ...
    def get_or_create(self, technology_name, group_id=None, parent_id=None):
        ...
        self.cursor.execute(
            "SELECT id FROM technologies WHERE name = ?", (technology_name,)
        )
        ...
        if row:
            ...
            self.cursor.execute(
                "INSERT INTO technologies (name, group_id, parent_id) VALUES (?, ?, ?)",
    ...
    def delete(self, technology_id):
        ...
        self.cursor.execute(
            "DELETE FROM task_required_skills WHERE technology_id = ?", (technology_id,)
        )
        self.cursor.execute(
            "DELETE FROM technician_technology_skills WHERE technology_id = ?",
            (technology_id,),
        )
    ...
    def get_or_create_group(self, group_name):
        ...
        self.cursor.execute(
            "SELECT id FROM technology_groups WHERE name = ?", (group_name,)
        )
        row = self.cursor.fetchone()
        if row:
            return row[0]  # Access by index to fix tuple error
            ...
            self.cursor.execute(
                "INSERT INTO technology_groups (name) VALUES (?)", (group_name,)
            )
            self.conn.commit()
            return self.cursor.lastrowid
    ...
    def get_all_groups(self):
        ...
        return [
            {"id": row["id"], "name": row["name"]} for row in self.cursor.fetchall()
        ]
...
class TaskManager:
    ...
    def remove_required_skill(self, task_id, technology_id):
        ...
        self.cursor.execute(
            "DELETE FROM task_required_skills WHERE task_id = ? AND technology_id = ?",
            (task_id, technology_id),
        )
        self.conn.commit()
    ...
    def remove_all_required_skills(self, task_id):
        ...
        self.cursor.execute(
            "DELETE FROM task_required_skills WHERE task_id = ?", (task_id,)
        )
        self.conn.commit()
...
def log_technician_skill_update(
    conn,
    technician_id,
    technology_id,
    task_id,
...
class TechnicianGroupManager:
    ...
    def get_or_create_group(self, group_name):
        self.cursor.execute(
            "SELECT id FROM technician_groups WHERE name = ?", (group_name,)
        )
        row = self.cursor.fetchone()
        if row:
            return row["id"]
            ...
            self.cursor.execute(
                "INSERT INTO technician_groups (name) VALUES (?)", (group_name,)
            )
            self.conn.commit()
            return self.cursor.lastrowid
    ...
    def update_group(self, group_id, new_name):
        self.cursor.execute(
            "UPDATE technician_groups SET name = ? WHERE id = ?", (new_name, group_id)
        )
        self.conn.commit()
    ...
    def delete_group(self, group_id):
        ...
        self.cursor.execute(
            "DELETE FROM technician_group_members WHERE group_id = ?", (group_id,)
        )
...
class LineConditionManager:
    ...
    def get_all_conditions(self):
        ...
        self.cursor.execute(
            ...
            "FROM line_conditions ORDER BY name"
        )
        return [dict(row) for row in self.cursor.fetchall()]
    ...
    def delete_condition(self, condition_id):
        ...
        Cascade delete handled by DB foreign key on task_line_conditions table.
        """
        self.cursor.execute(
            "DELETE FROM line_conditions WHERE id = ?",
            (condition_id,),
        )
        self.conn.commit()
        return self.cursor.rowcount > 0
    ...
    def assign_condition_to_task(self, task_id, condition_id):
        ...
        try:
            self.cursor.execute(
                "INSERT OR IGNORE INTO task_line_conditions "
                "(task_id, condition_id) VALUES (?, ?)",
