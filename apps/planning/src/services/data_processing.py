...
def sanitize_data(data, logger=None):  # Added logger parameter
    ...
    numeric_fields_to_int = [
        "planned_worktime_min",
        "mitarbeiter_pro_aufgabe",
        "quantity",
    ]
    ...
    for idx, row in enumerate(data):
        ...
        task_name_original = sanitized_row.get(
            "name",
            sanitized_row.get(
                "scheduler_group_task", f"Unknown Task at sanitize index {idx}"
            ),
        )
        ...
        for field in required_fields:
            if (
                field not in sanitized_row
                or sanitized_row[field] is None
                or pd.isna(sanitized_row[field])
            ):
                default_val = "Unknown"  # General default
                if field == "scheduler_group_task":
                    default_val = f"Unknown Task Name {task_id_original}"
                elif field == "priority":
                    default_val = "C"
                elif field == "task_type":
                    default_val = "REP"  # Default to REP if type is missing
        ...
        if "name" not in sanitized_row or not sanitized_row["name"]:
            ...
            if (
                sanitized_row["name"] == f"Unknown Task Name {task_id_original}"
        ...
        for field in numeric_fields_to_int:
            ...
            if not is_valid_number(value):
                default_val = (
                    1 if field in ["mitarbeiter_pro_aufgabe", "quantity"] else 0
                )
                _log_sanitize(
                    "warning",
...
def validate_assignments_flat_input(assignments_list):
    ...
    for idx, assignment in enumerate(assignments_list):
        if not isinstance(assignment, dict):
            logger.warning(
                f"Warning: Invalid assignment at index {idx}: not a dictionary"
            )
            continue
        required_fields = [
            "technician",
        ...
        try:
            ...
            if start < 0 or duration < 0:
                ...
                continue
            if (
                not isinstance(assignment["instance_id"], str)
