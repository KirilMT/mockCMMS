...
def _assign_task_definition_to_schedule(
    ...
    if num_technicians_needed == 0:
        ...
        _log(
            logger,
            "warning",
            f"Task definition {task_name_excel} (ID: {task_id}) unassigned for all {quantity} instances: {reason}",
        )
        return
    ...
    if (
        ...
        try:
            ...
            _log(
                logger,
                "warning",
                f"  Warning ({task_type}): Invalid line format '{task_lines_str}' for task {task_name_excel}",
            )
    ...
    for instance_num in range(1, quantity + 1):
        ...
        if task_type == "PM" and not is_additional_task_flag:
            _log(
                logger,
                "debug",
                f"    (Helper) Assigning Standard PM instance: {instance_task_display_name}, Required Technology IDs: {task_technology_ids}",
            )
            ...
            if not task_technology_ids:
                ...
                unassigned_tasks_reasons_dict[instance_id_str] = (
                    last_known_failure_reason_for_instance
                )
                _log(
                    logger, "warning", f"      {last_known_failure_reason_for_instance}"
                )
                continue
            ...
            if not eligible_technicians_details_pm:
                ...
                _log(
                    logger,
                    "warning",
                    f"      {last_known_failure_reason_for_instance} for {instance_task_display_name}",
                )
                continue
            ...
            if (
                ...
                if under_resourced_tasks is not None:
                    is_already_added = any(
                        t["task_id"] == task_id for t in under_resourced_tasks
                    )
            ...
            tech_details_map_pm = {
                d["name"]: d for d in eligible_technicians_details_pm
            }
            ...
            if num_technicians_needed > 0 and len(sorted_eligible_tech_names_pm) > 0:
                ...
                possible_sizes_to_try = sorted(
                    list(unique_sizes),
                    key=lambda s: (abs(s - num_technicians_needed), s),
                )
            ...
            for r_actual_group_size in possible_sizes_to_try:
                for group_tuple in combinations(
                    ...
                    if not set(task_technology_ids).issubset(
                        group_skills_possessed_by_id
                    ):
                        continue
                    ...
                    for req_skill_id in task_technology_ids:
                        techs_with_this_skill_in_group = [
                            ...
                            if req_skill_id
                            in tech_details_map_pm[tech_name]["relevant_skills"]
                        ]
                        if techs_with_this_skill_in_group:
                            avg_level_for_skill = sum(
                                tech_details_map_pm[tech_name]["relevant_skills"][
                                    req_skill_id
                                ]
                    ...
                    for tech_name_in_group in group_tech_names:
                        for req_skill_id in task_technology_ids:
                            if (
                                req_skill_id
                                in tech_details_map_pm[tech_name_in_group][
                                    "relevant_skills"
                                ]
                            ):
                                total_skill_points_in_group_for_required_skills += (
                                    tech_details_map_pm[tech_name_in_group][
                                        "relevant_skills"
                                    ][req_skill_id]
                                )
                    ...
                    if count_of_possessed_required_skills_in_group > 0:
                        combined_avg_skill_level_group = (
                            total_skill_points_in_group_for_required_skills
                            / count_of_possessed_required_skills_in_group
                        )
                    ...
                    viable_groups_with_scores_pm.append(
                        {
                            ...
                            "workload": workload,
                            "size_diff": abs(
                                r_actual_group_size - num_technicians_needed
                            ),
                        }
                    )

            if (
                str(task_to_assign.get("priority", "C")).upper() == "A"
                and 0 < len(sorted_eligible_tech_names_pm) < num_technicians_needed
            ):
                _log(
                    logger,
                    "info",
                    f"Task {task_name_excel} is Prio 'A' with {len(sorted_eligible_tech_names_pm)}/{num_technicians_needed} skilled techs. Seeking helpers.",
                )
                ...
                for num_skilled in range(len(sorted_eligible_tech_names_pm), 0, -1):
                    ...
                    if (
                        num_helpers_needed <= 0
                        or len(all_helper_names) < num_helpers_needed
                    ):
                        continue

                    for skilled_group_tuple in combinations(
                        sorted_eligible_tech_names_pm, num_skilled
                    ):
                        ...
                        for helper_group_tuple in combinations(
                            all_helper_names, num_helpers_needed
                        ):
                            ...
                            for req_skill_id in task_technology_ids:
                                techs_with_this_skill_in_group = [
                                    tech_name
                                    for tech_name in skilled_names
                                    if req_skill_id
                                    in tech_details_map_pm[tech_name]["relevant_skills"]
                                ]
                                if techs_with_this_skill_in_group:
                                    avg_level_for_skill = sum(
                                        tech_details_map_pm[tech_name][
                                            "relevant_skills"
                                        ][req_skill_id]
                                        for tech_name in techs_with_this_skill_in_group
                                    ) / len(techs_with_this_skill_in_group)
                                    per_skill_avg_levels[req_skill_id] = (
                                        avg_level_for_skill
                                    )
                                else:
                                    per_skill_avg_levels[req_skill_id] = 0
                            ...
                            for tech_name_in_group in skilled_names:
                                for req_skill_id in task_technology_ids:
                                    if (
                                        ...
                                        total_skill_points_in_group_for_required_skills += tech_details_map_pm[
                                            tech_name_in_group
                                        ][
                                            "relevant_skills"
                                        ][
                            ...
                            workload = sum(
                                sum(
                                    ...
                                    for start, end, _ in technician_schedules[tn]
                                )
                                for tn in group_tech_names
                            )
                    ...
                    if any(
                        g.get("is_helper_group") for g in viable_groups_with_scores_pm
                    ):
                        break
            ...
            viable_groups_with_scores_pm.sort(
                key=lambda x: (
                    x["size_diff"],
                    tuple(
                        -x["per_skill_avg"].get(skill_id, 0)
                        for skill_id in sorted_req_skill_ids_for_sorting
                    ),
            ...
            if not viable_groups_with_scores_pm:
                if num_technicians_needed > 0:
                    ...
                    last_known_failure_reason_for_instance = (
                        "Failed to process 0-tech, 0-duration PM task (no dummy group)."
                    )
                else:
            ...
            for group_candidate_data in viable_groups_with_scores_pm:
                ...
                if (
                    base_duration > 0
                    and num_technicians_needed > 0
                    and current_actual_num_assigned > 0
                ):
                    current_effective_duration = (
                        base_duration * num_technicians_needed
                    ) / current_actual_num_assigned
                ...
                while search_start_time <= total_work_minutes:
                    if (
                        current_effective_duration == 0
                        and search_start_time > total_work_minutes
                    ):
                        break
                    if (
                        current_effective_duration > 0
                    ...
                    duration_to_check = (
                        1
                        if current_effective_duration == 0
                        else current_effective_duration
                    )

                    if current_effective_duration > 0 and (
                        search_start_time + current_effective_duration
                        > total_work_minutes
                    ):
                        remaining_time_in_shift = max(
                            0, total_work_minutes - search_start_time
                        )
                        min_acceptable_partial_duration = (
                            current_effective_duration * 0.75
                        )

                        if (
                            remaining_time_in_shift >= min_acceptable_partial_duration
                            and remaining_time_in_shift > 0
                        ):
                    ...
                    for tech_in_group_name in current_candidate_group:
                        if not all(
                            sch_end <= search_start_time
                            or sch_start >= search_start_time + duration_to_check
                    ...
                    if all_techs_in_group_available_at_slot:
                        ...
                        final_assigned_duration_for_instance = (
                            duration_to_check if current_effective_duration > 0 else 0
                        )

                        final_is_helper_group = group_candidate_data.get(
                            "is_helper_group", False
                        )
            ...
            if assignment_successful_this_instance:
                ...
                if final_is_helper_group:
                    helpers_in_group = [
                        tech
                        for tech in final_chosen_group_for_instance
                        if tech not in tech_details_map_pm
                    ]
                    if helpers_in_group:
                        ...
                        _log(
                            logger,
                            "info",
                            f"Helper(s) assigned to task {task_name_excel} (ID: {task_id}): {helper_names_str}",
                        )
                        try:
                            cursor = db_conn.cursor()
                            for helper_name in helpers_in_group:
                                cursor.execute(
                                    "SELECT id FROM technicians WHERE name = ?",
                                    (helper_name,),
                                )
                                ...
                                for tech_id in task_technology_ids:
                                    cursor.execute(
                                        "SELECT skill_level FROM technician_technology_skills WHERE technician_id = ? AND technology_id = ?",
                                        (helper_id, tech_id),
                                    )
                                    ...
                                    if prev_level == 0:
                                        ...
                                        _log(
                                            logger,
                                            "info",
                                            f"Helper {helper_name} skill for technology {tech_id} updated from 0 to 1 due to assignment to {task_name_excel}",
                                        )
                            db_conn.commit()
                            ...
                            _log(
                                logger,
                                "warning",
                                f"Helper skill update/logging failed for task {task_name_excel} (ID: {task_id}): {e}",
                            )
                ...
                if num_technicians_needed > 0:
                    ...
                    num_technicians_needed == 0
                    and len(final_chosen_group_for_instance) > 0
                ):
                    ...
                    all_task_assignments_details.append(
                        {
                            ...
                            "is_incomplete": instance_id_str
                            in incomplete_tasks_instance_ids,
                ...
                _log(
                    logger,
                    "info",
                    f"    (Helper) Successfully scheduled PM {instance_task_display_name} for group {final_chosen_group_for_instance} at {final_start_time_for_instance} for {final_assigned_duration_for_instance} min. Incomplete: {instance_id_str in incomplete_tasks_instance_ids}. Required skills: {task_technology_ids}",
                )
            else:
                if (
                    not last_known_failure_reason_for_instance
                    or "Could not find a suitable time slot"
                    in last_known_failure_reason_for_instance
                    or "No viable technician groups"
                    in last_known_failure_reason_for_instance
                ):
                ...
                _log(
                    logger,
                    "warning",
                    f"      Failed to assign PM instance {instance_task_display_name}. Reason: {last_known_failure_reason_for_instance}",
                )
            ...
            if not assignment_info_rep:
                last_known_failure_reason_for_instance = (
                    "Skipped (REP): Task data not received from UI."
                )
                unassigned_tasks_reasons_dict[instance_id_str] = (
                    last_known_failure_reason_for_instance
                )
            ...
            if assignment_info_rep.get("skipped"):
                last_known_failure_reason_for_instance = assignment_info_rep.get(
            ...
            selected_tech_assignments_from_ui = assignment_info_rep.get(
                "technicians", []
            )
            raw_user_selection_count_rep = len(selected_tech_assignments_from_ui)

            selected_tech_names_from_ui = [
                tech["name"] for tech in selected_tech_assignments_from_ui
            ]

            eligible_user_selected_techs_rep = [
                tech_name
                for tech_name in selected_tech_names_from_ui
                if tech_name in present_technicians
                and (
                    not task_lines_list
                    or any(
                        line in TECHNICIAN_LINES.get(tech_name, [])
            ...
            forced_tech_names = {
                tech["name"]
                for tech in selected_tech_assignments_from_ui
                if tech.get("force_assign")
                and tech["name"] in eligible_user_selected_techs_rep
            }
            ...
            if not eligible_user_selected_techs_rep and num_technicians_needed > 0:
                last_known_failure_reason_for_instance = (
                    "Skipped (REP): None of the user-selected technicians are eligible."
                )
                unassigned_tasks_reasons_dict[instance_id_str] = (
                    last_known_failure_reason_for_instance
                )
            ...
            other_eligible_techs = [
                tech
                for tech in eligible_user_selected_techs_rep
                if tech not in forced_tech_names
            ]
            ...
            for r_size in range(len(other_eligible_techs) + 1):
                for other_group_tuple in combinations(other_eligible_techs, r_size):
                    ...
                    viable_groups_with_scores_rep.append(
                        {"group": group, "len": len(group), "workload": workload}
                    )
            ...
            viable_groups_with_scores_rep.sort(
                key=lambda x: (
            ...
            for group_candidate_data_rep in viable_groups_with_scores_rep:
                ...
                if (
                    base_duration > 0
                    and num_technicians_needed > 0
                    and current_actual_num_assigned_rep > 0
                ):
                    current_effective_duration_rep = (
                        base_duration * num_technicians_needed
                    ) / current_actual_num_assigned_rep
                ...
                if num_technicians_needed > 0:
                    ...
                    num_technicians_needed == 0 and current_actual_num_assigned_rep > 0
                ):
                ...
                while search_start_time_rep <= total_work_minutes:
                    if (
                        ...
                        and search_start_time_rep > total_work_minutes
                    ):
                        break
                    if (
                        current_effective_duration_rep > 0
                    ...
                    duration_to_check_for_slot_rep = (
                        1
                        if current_effective_duration_rep == 0
                        else current_effective_duration_rep
                    )
                    ...
                    for tech_in_group_name_rep in current_candidate_group_rep:
                        if not all(
                            sch_end <= search_start_time_rep
                            or sch_start
                            >= search_start_time_rep + duration_to_check_for_slot_rep
                            for sch_start, sch_end, _ in technician_schedules[
                                tech_in_group_name_rep
                            ]
                        ):
                            all_techs_in_rep_group_available = False
                            break
                    ...
                    if all_techs_in_rep_group_available:
                        final_chosen_group_for_rep_instance = (
                            current_candidate_group_rep
                        )
                        final_start_time_for_rep_instance = search_start_time_rep
                        assigned_duration_gantt_rep = current_effective_duration_rep

                        if current_effective_duration_rep > 0 and (
                            final_start_time_for_rep_instance
                            + current_effective_duration_rep
                            > total_work_minutes
                        ):
                            remaining_time = max(
                                0,
                                total_work_minutes - final_start_time_for_rep_instance,
                            )
                            min_acceptable_partial = (
                                current_effective_duration_rep * 0.75
                            )
                            if (
                                remaining_time >= min_acceptable_partial
                                and remaining_time > 0
                            ):
                                assigned_duration_gantt_rep = remaining_time
                                if instance_id_str not in incomplete_tasks_instance_ids:
                                    incomplete_tasks_instance_ids.append(
                                        instance_id_str
                                    )
                            else:
                                all_techs_in_rep_group_available = False
                        ...
                        if all_techs_in_rep_group_available:
                            final_assigned_duration_for_rep_instance = (
                                assigned_duration_gantt_rep
                            )
                            final_resource_mismatch_note_rep = (
                                current_resource_mismatch_note_rep_candidate
                            )
            ...
            if assignment_successful_this_instance_rep:
                ...
                if (
                    ...
                    last_known_failure_reason_for_instance = (
                        "No group/slot for REP task from UI selection."
                    )
                unassigned_tasks_reasons_dict[instance_id_str] = (
                    last_known_failure_reason_for_instance
                )
        ...
        if (
            not assigned_this_instance_flag
            and instance_id_str not in unassigned_tasks_reasons_dict
        ):
            unassigned_tasks_reasons_dict[instance_id_str] = (
                last_known_failure_reason_for_instance
            )
...
def assign_tasks(
    ...
    if technician_technology_skills is None:
        ...
        _log(
            logger,
            "warning",
            "Technician technology skills not provided to assign_tasks. Skill-based assignment will be limited.",
        )
        ...
        if task_type in ["PM", "REP"]:
            ...
            processed_task = {
                **task,
                ...
                "priority_val": priority_order.get(
                    str(task.get("priority", "C")).upper(), priority_order["DEFAULT"]
                ),
            }
    ...
    all_pm_task_names_from_excel_normalized_set = {
        ...
        for t in all_tasks_combined
        if t["task_type_upper"] == "PM"
    }
    ...
    if 0 < len(hp_tasks) <= MAX_PERMUTATION_TASKS:
        _log(
            logger,
            "info",
            f"Optimizing {len(hp_tasks)} high-priority tasks using permutations (limit: {MAX_PERMUTATION_TASKS}).",
        )
        ...
        if len(hp_tasks) > 0:
            num_permutations = 1
            for i in range(1, len(hp_tasks) + 1):
                num_permutations *= i
        ...
        for p_hp_task_list in permutations(hp_tasks):
            ...
            current_score = _calculate_hp_assignment_score(
                current_perm_assignments,
                hp_tasks,
                current_perm_unassigned_reasons,
                logger,
            ...
            if current_score > best_hp_overall_score:
                ...
                best_hp_overall_schedules = {
                    k: list(v) for k, v in current_perm_schedules.items()
                }
                best_hp_overall_unassigned_reasons = dict(
                    current_perm_unassigned_reasons
                )
        ...
        _log(
            logger,
            "info",
            f"Best HP permutation score: {best_hp_overall_score}. Using this schedule for HP tasks.",
        )
        ...
        final_incomplete_tasks_instance_ids.extend(
            iid
            for iid in best_hp_overall_incomplete_ids
            if iid not in final_incomplete_tasks_instance_ids
        )
        ...
        if len(hp_tasks) > MAX_PERMUTATION_TASKS:
            _log(
                logger,
                "info",
                f"Number of high-priority tasks ({len(hp_tasks)}) > {MAX_PERMUTATION_TASKS}. Assigning HP tasks greedily.",
            )
            hp_tasks.sort(
                key=lambda t: (
            ...
            _log(
                logger,
                "info",
                "Greedy HP tasks re-sorted by num_techs (desc), duration (desc), id (asc).",
            )
        elif not hp_tasks:
            _log(
                logger, "info", "No high-priority tasks to optimize with permutations."
            )
    ...
    _log(
        logger,
        "info",
        "Other-priority tasks re-sorted by prio (asc), num_techs (desc), duration (desc), id (asc).",
    )
    ...
    final_available_time_summary_map = {
        tech: total_work_minutes for tech in present_technicians
    }
    for tech_name_final, schedule_entries_final in final_technician_schedules.items():
        total_scheduled_time_for_tech = sum(
            end - start for start, end, _ in schedule_entries_final
        )
        final_available_time_summary_map[
            tech_name_final
        ] -= total_scheduled_time_for_tech
    ...
    _log(
        logger,
        "info",
        f"Unified task assignment process completed. Assigned {len(final_all_task_assignments_details)} task segments.",
    )
    if final_unassigned_tasks_reasons_dict:
        _log(
            logger,
            "warning",
            f"Unassigned task instances: {len(final_unassigned_tasks_reasons_dict)}. Reasons (sample):",
        )
        count = 0
        for inst_id, reason in final_unassigned_tasks_reasons_dict.items():
            ...
            if count >= 10 and len(final_unassigned_tasks_reasons_dict) > 15:
                _log(
                    logger,
                    "warning",
                    f"  ... and {len(final_unassigned_tasks_reasons_dict) - count} more unassigned instances.",
                )
                break
    ...
    if final_incomplete_tasks_instance_ids:
        _log(
            logger,
            "info",
            f"Incomplete task instances (due to shift end): {len(final_incomplete_tasks_instance_ids)} -> {final_incomplete_tasks_instance_ids}",
        )
    ...
    if under_resourced_tasks:
        _log(
            logger,
            "warning",
            f"Under-resourced PM tasks detected: {under_resourced_tasks}",
        )
...
def balance_workload_with_helpers(
    ...
    _log(
        logger,
        "info",
        f"Overloaded techs (>{overloaded_threshold / 60:.1f}h scheduled): {overloaded_techs}",
    )
                ...
                for idle_tech in idle_techs:
                    ...
                    if can_help:
                        _log(
                            logger,
                            "info",
                            f"Found helper '{idle_tech}' for task '{task_assignment['task_name']}' of technician '{overloaded_tech}'",
                        )
                        ...
                        if is_overloaded_tech_free and is_idle_tech_free:
                            ...
                            _log(
                                logger,
                                "info",
                                f"Task '{task_assignment['task_name']}' rescheduled with helper '{idle_tech}'.",
                            )
                            # Move to next task
                            break
                else:
    ...
    for tech in present_technicians:
        ...
        _log(
            logger,
            "debug",
            f"  - {tech}: {occupied_time:.2f} minutes occupied ({percentage_occupied:.2f}%)",
        )
...
def _get_technician_groups(db_conn):
    ...
    # Get group members
    cursor.execute(
        "SELECT g.name as group_name, t.name as tech_name FROM technician_group_members gm JOIN technicians t ON gm.technician_id = t.id JOIN technician_groups g ON gm.group_id = g.id"
    )
