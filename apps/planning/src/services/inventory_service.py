...
def check_spare_parts_availability(
    ...
    for part in maintenance_order.required_spare_parts:
        ...
        stmt = select(maintenance_order_spare_parts.c.quantity_required).where(
            maintenance_order_spare_parts.c.maintenance_order_id
...
def get_tasks_with_insufficient_parts(tasks: list[MaintenanceOrder]) -> dict:
    """Filters a list of maintenance orders and returns a dictionary of tasks that
    cannot be planned due to insufficient spare parts.
