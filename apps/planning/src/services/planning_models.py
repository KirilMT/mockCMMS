...
class PlanningTask(db.Model):  # type: ignore
    ...
    )  # Calculated duration after planning (may differ from MO estimate)
    status = Column(
        String(50), default="Unplanned"
    )  # Unplanned, Planned, In-Progress, Completed, Cancelled
    ...
    maintenance_order = relationship(
        "MaintenanceOrder",
        primaryjoin="foreign(PlanningTask.maintenance_order_id) == MaintenanceOrder.id",
    )
