...
class PlanningEngine:
    ...
    This class encapsulates the algorithm for assigning maintenance tasks to technicians
    based on skills, availability, and workload balancing.
    """
    ...
    def _get_weekend_shifts(
        self, start_date: datetime, end_date: datetime
    ) -> List[ShiftDefinition]:
        """Generate specific shift instances for the given date range based on Shift
        Patterns.
        ...
        while curr_date <= end_date_limit + timedelta(
            days=1
        ):  # +1 buffer for overnight
    ...
    def _get_working_teams(self, date: datetime) -> List:
        """Determine which shift teams are working for the given date based on 2-week
        rotation.
    ...
    def generate_plan(
        ...
        if planning_mode == "shift_break":
            max_task_duration = (
                max_task_duration or 30
            )  # 30-minute default for shift breaks
            self._log(
                "info", f"Shift-break mode: max task duration = {max_task_duration} min"
            )
        else:  # weekend mode
            max_task_duration = (
        ...
        self._log(
            "info", f"Found {len(tasks_to_plan)} plannable tasks before mode filtering"
        )
        ...
        self._log(
            "info",
            f"Found {len(tasks_to_plan)} tasks and {len(all_technicians)} technicians",
        )
        ...
        for shift in shifts:
            ...
            self._log(
                "info", f"Shift: {shift.name} -> Working Teams: {allowed_team_names}"
            )
        ...
        self._log(
            "info",
            f"Planning complete: {len(result.assigned_tasks)} assigned, "
    ...
    def _get_plannable_tasks(
        self, schedule: Schedule, check_parts: bool, result: PlanningResult
        ...
        for task in tasks:
            ...
            if not mo:
                result.add_warning(
                    f"PlanningTask {task.id} has no associated MaintenanceOrder"
                )
                continue
                ...
                if not is_available:
                    result.add_unassigned(
                        self._create_unassigned_task(
                            task,
                            mo,
                            UnassignedReason.INSUFFICIENT_PARTS,
        ...
        for task, mo in tasks:
            ...
            if mo.order_type == "PM" and mo.frequency:
                if mo.frequency.lower() in [
                    ...
                    self._log(
                        "debug",
                        f"MO-{mo.id}: Included (PM with frequency={mo.frequency})",
                    )
                    continue
                else:
                    ...
                    self._log(
                        "debug",
                        f"MO-{mo.id}: Filtered out (PM with frequency={mo.frequency})",
                    )
                    continue
            ...
            self._log(
                "debug",
                f"MO-{mo.id}: Filtered out (no weekend criteria match: {reason})",
            )

        if filtered_count > 0:
            reason_summary = ", ".join(
                [f"{count} {reason}" for reason, count in filter_reasons.items()]
            )
            result.add_warning(
    ...
    def _initialize_workloads(
        self, technicians: List[User], shift_duration_minutes: int
    ...
    def _assign_single_task(
        ...
        if required_skills and not self._team_has_all_skills(
            selected_technicians, required_skills
        ):
            return None
    ...
    def _find_eligible_technicians(
        ...
        if not required_skills:
            ...
            return [
                ...
                if self._has_available_time(
                    t, workloads, task_duration, shift_tech_busy
                )
            ]
        ...
        for tech in technicians:
            ...
            if self._has_required_skills(tech, required_skills):
                ...
                if self._has_available_time(
                    tech, workloads, task_duration, shift_tech_busy
                ):
    ...
    def _has_required_skills(
        self, technician: User, required_skills: List[str]
    ) -> bool:
        """Check if technician has all required skills."""
    ...
    def _has_available_time(
        ...
        remaining_time = (
            workload.total_available_minutes - workload.total_assigned_minutes
        )
    ...
    def _find_team_with_skill_coverage(
        ...
        for tech in available_techs:
            tech_skills = (
                {ts.skill.name for ts in tech.skills}
        ...
        while len(selected) < team_size and remaining_pool:
            ...
            for tech in remaining_pool:
                ...
                new_coverage = len(
                    tech_skills.intersection(required_skills) - covered_skills
                )
            ...
            if best_tech:
                ...
                tech_skills = (
                    {ts.skill.name for ts in best_tech.skills}
                    if hasattr(best_tech, "skills")
                    else set()
                )
    ...
    def _team_has_all_skills(
        self, team: List[User], required_skills: List[str]
    ) -> bool:
    ...
    def _select_best_team(
        ...
        """Select the best team from eligible technicians using advanced team formation
        logic.
        ...
        for tech in eligible_technicians:
            ...
            time_score = (
                remaining_time / workload.total_available_minutes
                if workload.total_available_minutes > 0
                else 0
            )
            ...
            max_skills = max(
                len(t.skills) if hasattr(t, "skills") else 0
            ...
            if hasattr(tech, "skills") and tech.skills:
                avg_skill_level = sum(ts.skill_level for ts in tech.skills) / len(
                    tech.skills
                )
                skill_level_score = (
                    avg_skill_level / 5.0
            ...
            total_score = (
                time_score * 0.40 + skill_score * 0.30 + skill_level_score * 0.30
            )
    ...
    def _balance_team_experience(
        self, scored_technicians: List[Tuple[User, float, int, float]], team_size: int
    ) -> List[User]:
        """Balance team composition with mix of experienced and less experienced
        technicians.
    ...
    def _calculate_adjusted_duration(
        self, base_duration: int, required_team_size: int, actual_team_size: int
    ) -> int:
        ...
        return max(
            adjusted_duration, int(base_duration * 0.5)
        )  # Never less than 50% of original

    def _fits_time_window(
        self, task_duration: int, max_allowed: int, mode: str
    ) -> bool:
...
def generate_plan(
    schedule_id: int, planning_mode: str = "weekend", logger=None
) -> PlanningResult:
