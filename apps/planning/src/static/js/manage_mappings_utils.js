...
function getTechnologyHierarchyPath(technologyId, allTechnologies) {
  ...
  console.warn(
    "getTechnologyHierarchyPath is a placeholder and needs to be implemented.",
  );
...
function recordChange(
  ...
  if (oldValue === undefined && newValue === undefined) {
    ...
    if (
      ...
      !type.toLowerCase().includes("remove")
    ) {
      return;
    }
  }
  ...
  console.log(
    "Change recorded:",
    window.unsavedChanges[window.unsavedChanges.length - 1],
  );
...
function sortAndRecalculatePriorities(tasksArray) {
  ...
  tasksArray.forEach((task, index) => {
    if (typeof task.user_prio === "undefined" || task.user_prio === null) {
      task.user_prio =
        task.prio !== undefined && task.prio !== null
          ? task.prio
          : index + 1000;
    }
  });
...
window.calculateAndAssignDisplayPriorities =
  calculateAndAssignDisplayPriorities;
