...
async function fetchTechnicianSkillUpgradeLogs(technicianId) {
  try {
    const response = await fetch(
      `/planning/api/technician_skill_upgrade_logs/${technicianId}`,
    );
...
async function fetchTechnicianSkills(technicianName) {
  ...
  try {
    ...
    const response = await fetch(
      `/planning/api/technician_skills/${currentSelectedTechnicianId}`,
    );
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    ...
    displayMessage(
      `Error fetching skills for ${technicianName}: ${error.message}`,
      "error",
    );
    console.error("Error fetching skills:", error);
    if (
      currentMappings.technicians &&
      currentMappings.technicians[technicianName]
    ) {
      currentMappings.technicians[technicianName].skills = {};
    }
    renderTechnicianSkills();
...
function renderTechnicianSkills() {
  ...
  if (!selectedTechnician || !currentMappings.technicians[selectedTechnician]) {
    technicianSkillsListContainerDiv.innerHTML =
      "<p>Select a technician to view their skills.</p>";
    return;
  }
  if (allTechnologies.length === 0) {
    technicianSkillsListContainerDiv.innerHTML =
      "<p>No technologies have been defined. Skills cannot be assigned.</p>";
    return;
  }
  ...
  const hasChildren = (technologyId) =>
    allTechnologies.some((tech) => tech.parent_id === technologyId);
  ...
  const createSkillViewMode = (technology, currentLevel) => {
    ...
    if (hasChildren(technology.id)) {
      ...
      parentSkillText.classList.add(
        "skill-level-text",
        "parent-skill-indicator",
      );
      parentSkillText.textContent = "N/A (Parent Skill)";
      parentSkillText.title =
        "This technology is a parent and cannot have a skill level directly assigned.";
    ...
    if (hasChildren(technology.id)) {
      if (typeof techSkills[technology.id] !== "undefined") {
        displayMessage(
          `Skill '${escapeHtml(technology.name)}' for ${escapeHtml(
            selectedTechnician,
          )} is now a parent skill and its level is no longer applicable.`,
          "warning",
        );
      }
    }
  ...
  const createSkillEditMode = (technology, currentLevel) => {
    ...
    skillSelect.addEventListener("change", async () => {
      ...
      await updateTechnicianSkill(
        selectedTechnician,
        currentSelectedTechnicianId,
        technology.id,
        newLevel,
      );
    });
  ...
  function renderSkillNode(parentElement, technology, level) {
    ...
    if (!hasChildren(technology.id)) {
      const currentSkillLevel =
        techSkills[technology.id] !== undefined
          ? parseInt(techSkills[technology.id], 10)
          : 0;
      controlsDiv.appendChild(
        createSkillViewMode(technology, currentSkillLevel),
      );
    } else {
    ...
    const children = allTechnologies.filter(
      (tech) => tech.parent_id === technology.id,
    );
...
async function updateTechnicianSkill(
  technicianName,
  technicianId,
  technologyId,
  skillLevel,
  ...
  try {
    const response = await fetch("/planning/api/technician_skill", {
      method: "POST",
    ...
    if (response.ok) {
      const technology = allTechnologies.find(
        (t) => t.id === parseInt(technologyId),
      );
      const technologyName = technology ? technology.name : "Unknown Skill";
      const techNameForMsg = technicianName || "Selected Technician";

      displayMessage(
        `Skill '${escapeHtml(technologyName)}' for technician '${escapeHtml(
          techNameForMsg,
        )}' updated to level ${escapeHtml(
          SKILL_LEVEL_TEXTS[skillLevel] || skillLevel,
        )}.`,
        "success",
      );
      ...
      if (currentMappings.technicians[technicianName]) {
        ...
        currentMappings.technicians[technicianName].skills[technologyId] =
          skillLevel;

        // We don't refetch logs, but we can clear the old one for the updated skill if we want.
        if (
          technicianSkillUpgradeLogs &&
          technicianSkillUpgradeLogs[technologyId]
        ) {
        ...
        console.error(
          "Could not find technician in local mappings to update skill.",
        );
