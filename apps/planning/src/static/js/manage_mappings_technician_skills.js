...
async function fetchTechnicianSkillUpgradeLogs(technicianId) {
    try {
        const response = await fetch(`/planning/api/technician_skill_upgrade_logs/${technicianId}`);
...
async function fetchTechnicianSkills(technicianName) {
    ...
    try {
        ...
        const response = await fetch(`/planning/api/technician_skills/${currentSelectedTechnicianId}`);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
