...
const hasChildren = (technologyId) => {
  ...
  if (!Array.isArray(allTechnologies)) {
    console.error(
      "hasChildren: allTechnologies is not an array or not available.",
    );
...
function getTechnologyPath(technologyId) {
  const tech = allTechnologies.find(
    (t) => String(t.id) === String(technologyId),
  );
  if (!tech) {
  ...
  while (current.parent_id) {
    const parentTech = allTechnologies.find(
      (t) => String(t.id) === String(current.parent_id),
    );
    if (!parentTech) {
...
function filterTechnologySelect(selectElement, searchTerm) {
  ...
  allOptions.forEach((opt) => {
    if (opt.value !== "") {
      // Don't hide functional placeholders if they were to exist
  ...
  initiallyMatchedOptions.forEach((matchedOption) => {
    ...
    if (
      currentOpt.parentElement &&
      currentOpt.parentElement.tagName === "OPTGROUP"
    ) {
    ...
    while (currentLevel > 0) {
      ...
      while (prevSibling) {
        if (
          prevSibling.tagName === "OPTION" &&
          parseInt(prevSibling.dataset.level) === parentLevelToFind
        ) {
          optionsToMakeVisible.add(prevSibling); // Add parent to display set
          if (
            prevSibling.parentElement &&
            prevSibling.parentElement.tagName === "OPTGROUP"
          ) {
        ...
        if (prevSibling.tagName === "OPTGROUP") {
          // Crossed an optgroup boundary upwards
  ...
  optionsToMakeVisible.forEach((opt) => {
    opt.style.display = "";
  });
  optgroupsToMakeVisible.forEach((og) => {
    og.style.display = "";
  });
  ...
  if (selectElement.multiple) {
    ...
    if (placeholderOption) {
      const anyOtherOptionVisible = Array.from(optionsToMakeVisible).some(
        (opt) => opt.value !== "",
      );
      if (optionsToMakeVisible.size === 0 || !anyOtherOptionVisible) {
        // If only placeholder would be visible or no options at all
...
function populateTechnologySelectDropdown(
  selectElement,
  selectedTechnologyValues = null,
) {
  // Renamed for clarity, can be single ID or array
  ...
  noTechOption.textContent = selectElement.multiple
    ? "-- Select Technologies --"
    : "-- Select Technology (Required) --";
  if (!selectElement.multiple) {
    // Only add "no selection" option for single select
  ...
  allTechnologies.forEach((t) => {
    if (t.parent_id) {
      if (!childrenByParentId[t.parent_id])
        childrenByParentId[t.parent_id] = [];
  ...
  function appendOptionsRecursive(
    parentElement,
    technologyId,
    level,
    ...
    children.forEach((childTech) => {
      ...
      option.innerHTML = `${"&nbsp;&nbsp;&nbsp;&nbsp;".repeat(
        level,
      )}↳ ${escapeHtml(childTech.name)}`;
      option.dataset.level = level; // Add data-level
      if (currentSelectedValues) {
        if (
          Array.isArray(currentSelectedValues) &&
          currentSelectedValues.map(String).includes(String(childTech.id))
        ) {
  ...
  sortedGroupNames.forEach((groupName) => {
    ...
    topLevelTechsInGroup.forEach((tech) => {
      ...
      if (selectedTechnologyValues) {
        if (
          Array.isArray(selectedTechnologyValues) &&
          selectedTechnologyValues.map(String).includes(String(tech.id))
        ) {
          option.selected = true;
...
async function addNewTaskForMapping() {
  ...
  const selectedTechOptions = Array.from(
    newTaskTechnologySelectForMapping.selectedOptions,
  );
  const technologyIds = selectedTechOptions
  ...
  if (technologyIds.length === 0) {
    displayMessage(
      "At least one technology must be selected for the new task.",
      "error",
    );
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      displayMessage(
        `Task '${escapeHtml(result.name)}' added successfully.`,
        "success",
      );
      newTaskNameForMappingInput.value = "";
      // Reset multi-select dropdown
      Array.from(newTaskTechnologySelectForMapping.options).forEach(
        (option) => (option.selected = false),
      );
      if (
        newTaskTechnologySelectForMapping.options.length > 0 &&
        !newTaskTechnologySelectForMapping.multiple
      ) {
      ...
      displayMessage(
        result.message || `Error adding task: Server error ${response.status}`,
        "error",
      );
    }
  } catch (error) {
    displayMessage(
      `Failed to add task. Network error or invalid response.`,
      "error",
    );
...
async function fetchAllTasksForMapping() {
  if (allTechnologies.length === 0) {
    taskTechnologyMappingListContainerDiv.innerHTML =
      "<p>Waiting for technologies to load before fetching task mappings...</p>";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Error fetching tasks for mapping. Status: ${response.status}, StatusText: ${response.statusText}, ServerResponse: ${errorText}`,
      );
      throw new Error(
        `HTTP error! status: ${response.status}. Server said: ${response.statusText}`,
      );
    }
    ...
    displayMessage(
      `Error fetching tasks for mapping: ${error.message}`,
      "error",
    );
    console.error(
      "Full error details in fetchAllTasksForMapping catch block:",
      error,
    );
    taskTechnologyMappingListContainerDiv.innerHTML =
      "<p>Error loading tasks. Check console and ensure server is running correctly with the latest API endpoints.</p>";
  }
}
...
function renderTasksForTechnologyMapping(tasks) {
  ...
  if (!tasks || tasks.length === 0) {
    taskTechnologyMappingListContainerDiv.innerHTML =
      "<p>No tasks found to map. Add a new task above.</p>";
    return;
  }

  if (allTechnologies.length === 0) {
    taskTechnologyMappingListContainerDiv.innerHTML =
      "<p>Technologies not loaded yet. Cannot map tasks.</p>";
    if (
      newTaskTechnologySelectForMapping &&
      newTaskTechnologySelectForMapping.options.length <= 1
    ) {
      // Check if it has more than the placeholder
  ...
  tasks.forEach((task) => {
    ...
    if (task.technology_ids && task.technology_ids.length > 0) {
      if (allTechnologies.length > 0) {
        ...
        if (currentTechDisplayPaths.length > 0) {
          finalDisplayMessageForTechSpan = `(${currentTechDisplayPaths.join(
            "<br />",
          )})`;
        ...
        // Technologies not loaded, but task has technology_ids
        hasAnyInvalidAssignments = true;
      ...
      // No technology_ids for the task
    ...
    saveBtn.onclick = async () => {
      ...
      const newTechnologyIds = selectedTechOptions
        .map((opt) => parseInt(opt.value))
        .filter((id) => !isNaN(id));
  ...
  const newTaskTechnologySearchInput = document.getElementById(
    "newTaskTechnologySearch",
  );
  if (newTaskTechnologySearchInput) {
    newTaskTechnologySearchInput.addEventListener("input", () => {
      filterTechnologySelect(
        newTaskTechnologySelectForMapping,
        newTaskTechnologySearchInput.value,
      );
    });
  }
}

async function updateTaskMapping(taskId, newName, newTechnologyIds) {
  // Changed to newTechnologyIds (array)
  try {
    ...
    if (response.ok && isJson) {
      displayMessage(
        `Task '${escapeHtml(result.name)}' updated successfully.`,
        "success",
      );
    } else if (isJson) {
      displayMessage(
        `Error updating task: ${escapeHtml(result.message || "Unknown error")}`,
        "error",
      );
    } else {
...
async function deleteTaskMapping(taskId, taskName) {
  if (
    !confirm(
      `Are you sure you want to delete task \"${escapeHtml(
        taskName,
      )}\"? This will also remove its assignments to technicians.`,
    )
  ) {
    return;
  }
  try {
    ...
    if (response.ok) {
      displayMessage(
        `Task \"${escapeHtml(taskName)}\" deleted successfully.`,
        "success",
      );
      await fetchAllTasksForMapping();
      ...
      const result = await response
        .json()
        .catch(() => ({ message: "Failed to parse error message." })); // Try to parse error
