...
class PlanningGanttChart {
    ...
    if (planningMode === "shift_break") {
      ...
      if (this.tasks.length > 0) {
        earliestTime = new Date(
          Math.min(...this.tasks.map((t) => new Date(t.planned_start_time))),
        );
      ...
      earliestTime.setMinutes(
        Math.floor(earliestTime.getMinutes() / 30) * 30,
        0,
        0,
      );
      ...
      if (this.tasks.length > 0) {
        ...
        latestTime = new Date(
          Math.max(...this.tasks.map((t) => new Date(t.planned_end_time))),
        );
      } else {
  ...
  renderGanttStructure(startTime, endTime, timeColumns, tasksByTech) {
    const techsWithTasks = this.technicians.filter(
      (tech) => tasksByTech[tech.id],
    );
    const techsToShow =
      techsWithTasks.length > 0 ? techsWithTasks : this.technicians;
  ...
  renderTechnicianLabels(tasksByTech) {
    ...
    if (techsWithTasks.length === 0) {
      ...
      techsWithTasks.forEach((tech) => {
        ...
        html += `
                    <div class="gantt-tech-label" data-tech-id="${
                      tech.id
                    }" style="height: 40px; padding: 0 10px; display: flex; align-items: center; border-bottom: 1px solid #eee; background: #f9f9f9;">
                        <span style="font-weight: 500;">${tech.name}</span>
                        <span style="margin-left: auto; font-size: 0.85em; color: #666;">${taskCount} task${
                          taskCount > 1 ? "s" : ""
                        }</span>
                    </div>
                `;
  ...
  renderTimeAxis(timeColumns) {
    ...
    shiftCells = shiftGroups
      .map(
        (group) => `
            <div class="gantt-header-cell" style="width: ${
              group.count * 100
            }px; text-align: center; padding: 4px 0; border-right: 1px solid #ddd; border-bottom: 1px solid #eee; font-size: 0.85em; font-weight: 500; background: ${
              group.label.includes("Late") ? "#f8f9fa" : "#fff"
            }; flex-shrink: 0;">
                ${group.label}
            </div>
        `,
    ...
    dayCells = dayGroups
      .map(
        (group) => `
            <div class="gantt-header-cell" style="width: ${
              group.count * 100
            }px; text-align: center; padding: 4px 0; border-right: 1px solid #ccc; border-bottom: 1px solid #eee; font-weight: bold; background: #e9ecef; flex-shrink: 0;">
                ${group.label}
            </div>
        `,
      )
  ...
  renderGanttRows(startTime, endTime, timeColumns, tasksByTech) {
    ...
    techsToShow.forEach((tech) => {
      ...
      html += `
                <div class="gantt-row" data-tech-id="${
                  tech.id
                }" style="position: relative; height: 40px; border-bottom: 1px solid #eee;">
                    <div class="gantt-time-grid" style="position: absolute; top: 0; left: 0; right: 0; height: 100%; display: grid; grid-template-columns: repeat(${
                      timeColumns.length
                    }, 100px);">
  ...
  changeViewMode(mode) {
    ...
    alert(
      `View mode "${mode}" will be implemented in a future update.Currently showing hourly view.`,
    );
  }
