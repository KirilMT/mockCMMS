...
async function initializePage() {
  ...
  if (technicianSelect) {
    technicianSelect.addEventListener("change", async (event) => {
      if (unsavedChanges && selectedTechnician) {
        ...
        if (changesText) changesText = `- ${changesText}`;
        else changesText = "General modifications.";
        if (
          !confirm(
  ...
  if (addTaskBtn) {
    addTaskBtn.addEventListener("click", () => {
      ...
      const tasksArray =
        currentMappings.technicians[selectedTechnician].task_assignments || [];
      currentMappings.technicians[selectedTechnician].task_assignments =
        tasksArray;
      let maxUserPrio = 0;
  ...
  const editTechnicianNameBtn = document.getElementById(
    "editTechnicianNameBtn",
  );
  ...
  const newTechnicianNameInputElement = document.getElementById(
    "newTechnicianNameInput",
  );
  if (newTechnicianNameInputElement) {
    newTechnicianNameInputElement.addEventListener(
      "input",
      handleNewTechnicianNameInputChange,
    );
  }
  const newTechnicianForm = document.getElementById(
  ...
  const cancelNewTechnicianButton = document.getElementById(
  ...
  const editTechSatellitePointButton = document.getElementById(
    "editTechSatellitePointBtn",
  );
  if (editTechSatellitePointButton) {
    editTechSatellitePointButton.addEventListener(
      "click",
      handleTechSatellitePointEdit,
    ); // from manage_mappings_technician_data.js
  }
  const techSatellitePointSelectElement = document.getElementById(
    "techSatellitePointSelect",
  );
  if (techSatellitePointSelectElement) {
    techSatellitePointSelectElement.addEventListener(
      "change",
      handleTechSatellitePointChange,
    ); // from manage_mappings_technician_data.js
  }
  ...
  if (newTechnologyNameInput) {
    newTechnologyNameInput.addEventListener(
      "input",
      handleTechnologyNameInputChange,
    );
  }
  if (newTechnologyGroupSelect) {
    newTechnologyGroupSelect.addEventListener(
      "change",
      handleTechnologyGroupChange,
    );
  }
  ...
  document
    .getElementById("satellitePointForm")
    .addEventListener("submit", (event) => {
      event.preventDefault();
      handleAddSatellitePoint();
    });
  document
    .getElementById("productionLineForm")
    .addEventListener("submit", (event) => {
      event.preventDefault();
      handleAddLine();
    });
...
document.addEventListener("DOMContentLoaded", () => {
  initializePage().catch((error) => {
    displayMessage(
      "Page failed to load completely. Check console for errors.",
      "error",
    );
  });
});
