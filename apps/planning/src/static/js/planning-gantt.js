...
class PlanningGanttChart {
    ...
    async loadData() {
        try {
            const response = await fetch(`/planning/planning/schedules/${this.scheduleId}/gantt-data`);
            if (!response.ok) {
    ...
    transformToGanttFormat(data) {
        ...
        return plannedTasks.map((task, index) => {
            ...
            const techNames = task.assigned_technician_names && task.assigned_technician_names.length > 0
                ...
                custom_popup_html: (task) => {
                    ...
                    return `
                        <div class="gantt-popup">
                            ...
                            <p><strong>Type:</strong> ${data.task_type || 'N/A'}</p>
                            <p><strong>Assigned To:</strong> ${data.assigned_technician_names ? data.assigned_technician_names.join(', ') : 'Unassigned'}</p>
                            <p><strong>Duration:</strong> ${data.actual_duration_minutes || data.estimated_duration_minutes || 'N/A'} min</p>
                            ${data.required_skills && data.required_skills.length > 0 ?
                                `<p><strong>Skills:</strong> ${data.required_skills.join(', ')}</p>` : ''}
                        </div>
                    `;
                }
            });
