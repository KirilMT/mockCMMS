...
const techSatellitePointSelect = document.getElementById(
  "techSatellitePointSelect",
);
...
const addNewTechnicianFormContainer = document.getElementById(
  "addNewTechnicianFormContainer",
);
const newTechnicianNameInput = document.getElementById(
  "newTechnicianNameInput",
);
const newTechnicianSatellitePointSelect = document.getElementById(
  "newTechnicianSatellitePointSelect",
);
const saveNewTechnicianBtn = document.getElementById("saveNewTechnicianBtn");
const cancelNewTechnicianBtn = document.getElementById(
  "cancelNewTechnicianBtn",
);
const newTechnicianError = document.getElementById("newTechnicianError");

// --- Satellite Points Dropdown Population ---
async function fetchAndPopulateSatellitePointsDropdowns() {
  // Renamed to reflect multiple dropdowns
  try {
    const response = await fetch("/planning/api/satellite_points");
    if (!response.ok) {
    ...
    if (techSatellitePointSelect) {
      ...
      techSatellitePointSelect.innerHTML =
        '<option value="">Select Satellite Point</option>'; // Default option
    ...
    if (newTechnicianSatellitePointSelect) {
      newTechnicianSatellitePointSelect.innerHTML =
        '<option value="">Select Satellite Point</option>'; // Default option
      satellitePoints.forEach((sp) => {
        const option = document.createElement("option");
        option.value = sp.id;
...
async function fetchMappings(technicianNameToSelect = null) {
  try {
    const response = await fetch("/planning/api/get_technician_mappings");
    ...
    if (
      technicianNameToSelect &&
      data.technicians &&
      data.technicians[technicianNameToSelect]
    ) {
      finalTechnicianToLoad = technicianNameToSelect;
    } else if (
      oldSelectedValue &&
      data.technicians &&
      data.technicians[oldSelectedValue] &&
      oldSelectedValue !== ""
    ) {
      // check oldSelectedValue is not placeholder
    ...
    if (finalTechnicianToLoad) {
      ...
      if (typeof loadTechnicianDetails === "function") {
        ...
        console.error(
          "loadTechnicianDetails function is not defined. Cannot refresh technician view.",
        );
      ...
      if (technicianSkillsListContainerDiv)
    ...
    displayMessage(
      `Error fetching technician mappings: ${error.message}`,
      "error",
    );
...
async function loadTechnicianDetails(technicianName) {
  ...
  if (!techData) {
    displayMessage(
      `Details for ${escapeHtml(technicianName)} not found.`,
      "error",
    );
    selectedTechnician = null;
    currentSelectedTechnicianId = null;
  ...
  if (currentTechNameDisplay)
    currentTechNameDisplay.textContent = `Details for: ${escapeHtml(
      selectedTechnician,
    )}`;

  // Satellite Point Display and Edit Handling
  const techSatellitePointNameDisplay = document.getElementById(
    "techSatellitePointNameDisplay",
  );
  const editTechSatellitePointBtn = document.getElementById(
    "editTechSatellitePointBtn",
  );
  ...
  if (
    ...
    techSatellitePointNameDisplay.textContent =
      techData.satellite_point_name || "Not Set";
  ...
  const skillMatchedTasks = techData.skill_matched_tasks || {
    full_match: [],
    partial_match: [],
  };
  renderTaskCategory(
    skillMatchedTasks.full_match,
    "Full Skill Match",
    taskListDiv,
  );
  renderTaskCategory(
    skillMatchedTasks.partial_match,
    "Partial Skill Match",
    taskListDiv,
  );
  ...
  if (
    skillMatchedTasks.full_match.length === 0 &&
    skillMatchedTasks.partial_match.length === 0
  ) {
    const noTasksP = document.createElement("p");
    noTasksP.textContent =
      "No tasks found for which this technician possesses any of the required skills.";
...
async function handleTechSatellitePointEdit() {
  ...
  if (
    ...
    techSatellitePointSelect.value =
      currentMappings.technicians[selectedTechnician]?.satellite_point_id || "";
...
async function handleTechSatellitePointChange() {
  if (!selectedTechnician || !currentSelectedTechnicianId) {
    displayMessage(
      "No technician selected for saving satellite point.",
      "error",
    );
  ...
  const oldSatellitePointName =
    currentMappings.technicians[selectedTechnician]?.satellite_point_name ||
    "Not Set";
  const newSatellitePointId = parseInt(techSatSelect.value, 10);
  const newSatellitePointName =
    techSatSelect.options[techSatSelect.selectedIndex]?.text || "Not Set";

  const payload = {
    technicians: {
      [selectedTechnician]: {
        // selectedTechnician is the name
        id: currentSelectedTechnicianId,
        satellite_point_id:
          isNaN(newSatellitePointId) || newSatellitePointId <= 0
            ? null
            : newSatellitePointId,
      },
    },
  ...
  try {
    const response = await fetch("/planning/api/save_technician_mappings", {
      method: "POST",
    ...
    if (response.ok) {
      ...
      const successMessage = `Satellite point for ${escapeHtml(
        selectedTechnician,
      )} changed from '${escapeHtml(oldSatellitePointName)}' to '${escapeHtml(
        newSatellitePointName,
      )}'.`;
      ...
      // Update UI
      if (techSatellitePointNameDisplay)
        techSatellitePointNameDisplay.textContent =
          currentMappings.technicians[selectedTechnician]
            ?.satellite_point_name || "Not Set";
      if (techSatellitePointSelect)
        techSatellitePointSelect.style.display = "none";
      if (techSatellitePointNameDisplay)
        techSatellitePointNameDisplay.style.display = "inline";
      if (editTechSatellitePointBtn)
        editTechSatellitePointBtn.style.display = "inline";
    } else {
...
function handleNewTechnicianNameInputChange() {
  if (newTechnicianNameInput && newTechnicianSatellitePointSelect) {
    newTechnicianSatellitePointSelect.disabled =
      newTechnicianNameInput.value.trim() === "";
  }
}
...
async function handleSaveNewTechnician() {
  ...
  try {
    ...
    const response = await fetch("/planning/api/technicians", {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      displayMessage(
        `Technician '${escapeHtml(name)}' added successfully.`,
        "success",
      );
      ...
      window.displayMessage(
        result.message ||
          `Error adding technician: Server error ${response.status}`,
        "error",
      );
    }
  } catch (error) {
    window.displayMessage(
      `Failed to add technician. Network error or invalid response.`,
      "error",
    );
...
async function handleEditTechnicianName() {
  ...
  if (
    newNamePrompt &&
    newNamePrompt.trim() !== "" &&
    newNamePrompt.trim() !== oldName
  ) {
    const newName = newNamePrompt.trim();
    try {
      const response = await fetch(
        `/planning/api/technicians/${currentSelectedTechnicianId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        displayMessage(
          `Changed name of technician from '${escapeHtml(
            oldName,
          )}' to '${escapeHtml(newName)}'.`,
          "success",
        );
      ...
      displayMessage(
        `Error editing technician name: ${error.message}`,
        "error",
      );
  ...
  } else if (newNamePrompt !== null) {
    // null means cancel was hit
...
async function handleDeleteTechnician() {
  ...
  if (
    confirm(
      `Are you sure you want to delete technician "${escapeHtml(
        technicianNameToDelete,
      )}"? This will also remove their skills and task assignments.`,
    )
  ) {
    try {
      const response = await fetch(
