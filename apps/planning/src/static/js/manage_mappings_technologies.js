...
async function fetchAllTechnologies() {
  try {
    const response = await fetch("/planning/api/technologies");
...
function populateParentTechnologySelect() {
  ...
  if (techName === "") {
    ...
    newTechnologyParentSelect.innerHTML =
...
function populateParentTechnologySelectFiltered(selectedGroupId) {
  ...
  newTechnologyParentSelect.disabled =
    newTechnologyNameInput.value.trim() === ""; // Re-evaluate based on name input

  const groupIdInt = parseInt(selectedGroupId);
  const groupTechnologies = allTechnologies.filter(
    (tech) => tech.group_id === groupIdInt,
  );

  function addTechOptions(parentElement, currentParentIdInGroup, level) {
    const children = groupTechnologies.filter(
      (tech) => tech.parent_id === currentParentIdInGroup,
    );
  ...
  const topLevelInGroup = groupTechnologies.filter(
    (tech) =>
      tech.parent_id === null ||
      !groupTechnologies.some((parentTech) => parentTech.id === tech.parent_id),
  );
...
function renderAllTechnologies() {
  ...
  if (allTechnologies.length === 0) {
    technologyListContainerDiv.innerHTML =
      "<p>No technologies defined yet.</p>";
    return;
  }

  const topLevelTechnologies = allTechnologies.filter(
    (tech) => tech.parent_id === null,
  );
  topLevelTechnologies.sort((a, b) => {
    const groupCompare = (a.group_name || "ZZZ").localeCompare(
      b.group_name || "ZZZ",
    );
  ...
  topLevelTechnologies.forEach((topLevelTech) => {
    if (topLevelTech.group_name !== currentGroupName) {
      ...
      groupHeader.textContent = escapeHtml(
        topLevelTech.group_name || "Uncategorized",
      );
...
async function addNewTechnology() {
  ...
  if (!selectedGroupId) {
    displayMessage(
      "Please assign a technology group. This field is mandatory.",
      "error",
    );
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      displayMessage(
        `Technology '${escapeHtml(result.name)}' added.`,
        "success",
      );
      ...
      if (selectedTechnician) {
        // If a technician is selected, refresh their details
        await fetchMappings(selectedTechnician);
      }
    } else {
      displayMessage(
        result.message ||
          `Error adding technology: Server error ${response.status}`,
        "error",
      );
    }
  } catch (error) {
    displayMessage(
      `Failed to add technology. Network error or invalid response.`,
      "error",
    );
...
async function editTechnology(techId) {
  ...
  const techListItem = technologyListContainerDiv.querySelector(
    `.list-item[data-tech-id="${techId}"]`,
  );
  if (!techListItem) {
  ...
  allTechnologyGroups.forEach((group) => {
    const isSelected =
      parseInt(group.id) === parseInt(currentGroupId) ? "selected" : "";
    groupSelectHtml += `<option value="${group.id}" ${isSelected}>${escapeHtml(
      group.name,
    )}</option>`;
  });
  ...
  allTechnologies.forEach((tech) => {
    if (tech.id !== techId) {
      ...
      const isSelected =
        parseInt(tech.id) === parseInt(currentParentId) ? "selected" : "";
      parentSelectHtml += `<option value="${
        tech.id
      }" ${isSelected}>${escapeHtml(tech.name)}</option>`;
    }
  });
  parentSelectHtml += "</select>";

  techListItem.innerHTML = `
        <input type="text" id="editTechnologyName_${techId}" value="${escapeHtml(
          currentName,
  ...
  techListItem
    ...
    .addEventListener("click", async () => {
      ...
      const newParentId = techListItem.querySelectorAll("select")[1].value
        ? parseInt(techListItem.querySelectorAll("select")[1].value)
        : null;
      ...
      if (newName) {
        ...
        try {
          const response = await fetch(`/planning/api/technologies/${techId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
          ...
          if (response.ok) {
            displayMessage(
              `Technology '${escapeHtml(result.name)}' updated.`,
              "success",
            );
            // Explicitly update the technology in the allTechnologies array
            const updatedTechIndex = allTechnologies.findIndex(
              (t) => t.id === techId,
            );
          ...
          displayMessage(
            `Error updating technology: ${error.message}`,
            "error",
          );
  ...
  techListItem
    .querySelector(".cancel-edit-tech-btn")
...
async function deleteTechnology(techId) {
  ...
  if (
    !confirm(
      `Are you sure you want to delete \"${escapeHtml(
        techName,
      )}\"? This may affect child technologies, task mappings, and technician skills.`,
    )
  ) {
    // Removed ID
    return;
  ...
  try {
    ...
    if (response.ok) {
      ...
      displayMessage(
        result.message || `Technology \"${escapeHtml(techName)}\" deleted.`,
        "success",
      ); // Removed ID
      await fetchAllTechnologies();
      await fetchAllTasksForMapping();
...
async function fetchTechnologyGroups() {
  try {
    const response = await fetch("/planning/api/technology_groups");
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
...
function renderTechnologyGroups() {
  ...
  if (
    Array.from(newTechnologyGroupSelect.options).some(
      (opt) => opt.value === previousGroupValue,
    )
  ) {
...
async function editTechnologyGroup(groupId, currentName) {
  const groupListItem = technologyGroupListContainerDiv.querySelector(
    `.list-item[data-group-id="${groupId}"]`,
  );
  if (!groupListItem) {
  ...
  groupListItem.innerHTML = `
        <input type="text" name="groupName" class="form-control" value="${escapeHtml(
          currentName,
        )}" style="flex-grow: 1;"/>
        <div class="item-actions">
      ...
      if (newName && newName !== currentName) {
        try {
          const response = await fetch(
            `/planning/api/technology_groups/${groupId}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
          ...
          if (response.ok) {
            displayMessage(
              `Group updated to '${escapeHtml(newName)}'.`,
              "success",
            );
  ...
  groupListItem
    .querySelector(".cancel-edit-group-btn")
    .addEventListener("click", () => {
...
async function deleteTechnologyGroup(groupId) {
  ...
  if (
    !confirm(
      `Are you sure you want to delete technology group \"${escapeHtml(
        groupName,
      )}\"? This might affect associated technologies.`,
    )
  ) {
    // Removed ID
    return;
  ...
  try {
    const response = await fetch(`/planning/api/technology_groups/${groupId}`, {
      method: "DELETE",
    });
    const result = await response.json();
    if (response.ok) {
      displayMessage(
        result.message ||
          `Technology group \"${escapeHtml(groupName)}\" deleted.`,
        "success",
      ); // Removed ID
      fetchTechnologyGroups();
      fetchAllTechnologies();
    ...
    displayMessage(
      `Error deleting technology group: ${error.message}`,
      "error",
    );
    console.error(error);
  }
