...
async function loadSatellitePoints() {
  try {
    const response = await window.fetch_get("/planning/api/satellite_points");
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ message: "Invalid JSON response" }));
      window.displayMessage(
        `Error loading satellite points: ${
    ...
    window.displayMessage(
      "Failed to load satellite points. See console for details.",
      "error",
    );
  }
}
...
async function handleAddSatellitePoint() {
  ...
  try {
    const response = await window.fetch_post("/planning/api/satellite_points", {
      name,
    });
    ...
    if (!response.ok) {
      window.displayMessage(
        responseData.message ||
          `Error adding satellite point: Server error ${response.status}`,
        "error",
      );
    } else {
      window.displayMessage(
        `Satellite point '${window.escapeHtml(
          responseData.name,
        )}' added successfully.`,
        "success",
      );
    ...
    window.displayMessage(
      "Failed to add satellite point. Network error or invalid response.",
      "error",
    );
...
function handleEditSatellitePoint(event) {
  ...
  listItem.innerHTML = `
        <input type="text" id="editSatellitePointName_${pointId}" value="${window.escapeHtml(
          currentRawName,
        )}" class="edit-input form-control" data-id="${pointId}" style="flex-grow:1; margin-right: 5px;">
  ...
  listItem
    .querySelector(".save-edit-satellite-point")
  ...
  listItem
    .querySelector(".cancel-edit-satellite-point")
...
async function executeUpdateSatellitePoint(pointId, newName) {
  // newName is raw
  try {
    const response = await window.fetch_put(
      `/planning/api/satellite_points/${pointId}`,
      { name: newName },
    );
    ...
    if (!response.ok) {
      window.displayMessage(
        `Error updating satellite point: ${
    ...
    window.displayMessage(
      "Failed to update satellite point. See console for details.",
      "error",
    );
  } finally {
    loadSatellitePoints();
...
async function handleDeleteSatellitePoint(event) {
  ...
  const cleanedPointName =
    typeof rawPointName === "string"
      ? rawPointName.replace(/\"/g, '"')
      : rawPointName;

  if (
    !confirm(
      `Are you sure you want to delete satellite point "${cleanedPointName}"? This may affect associated production lines.`,
    )
  ) {
    return;
  }
  ...
  try {
    const response = await window.fetch_delete(
      `/planning/api/satellite_points/${pointId}`,
    );
    ...
    if (!response.ok) {
      window.displayMessage(
        `Error deleting satellite point: ${
          responseData.message || response.status
        }`,
        "error",
      );
    ...
    window.displayMessage(
      "Failed to delete satellite point. See console for details.",
      "error",
    );
...
async function loadLines() {
  try {
    const response = await window.fetch_get("/planning/api/lines");
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ message: "Invalid JSON response" }));
      window.displayMessage(
        `Error loading lines: ${errorData.message || response.status}`,
        "error",
      );
      return;
    }
    ...
    window.displayMessage(
      "Failed to load lines. See console for details.",
      "error",
    );
  }
}
...
function renderLines(lines) {
  ...
  if (!lines || lines.length === 0) {
    container.innerHTML =
      "<p>No lines found. Add lines below or ensure satellite points exist.</p>";
    return;
  }
...
async function handleAddLine() {
  ...
  const satellitePointSelect = document.getElementById(
    "newLineSatellitePointSelect",
  );
  ...
  if (!satellitePointId) {
    window.displayMessage(
      "Please select a satellite point for the line.",
      "error",
    );
    return;
  }
  ...
  try {
    const response = await window.fetch_post("/planning/api/lines", {
      name,
    ...
    if (!response.ok) {
      window.displayMessage(
        responseData.message ||
          `Error adding line: Server error ${response.status}`,
        "error",
      );
    } else {
      ...
      let successMsg;
      if (rawSpName) {
        successMsg = `Line '${lineNameStr}' added successfully to satellite point '${window.escapeHtml(
          rawSpName,
        )}'.`;
      } else {
    ...
    window.displayMessage(
      "Failed to add line. Network error or invalid response.",
      "error",
    );
...
function handleEditLine(event) {
  ...
  const sourceSpDropdown = document.getElementById(
    "newLineSatellitePointSelect",
  );
  if (sourceSpDropdown) {
    ...
    spSelectElement.appendChild(option);
    window.displayMessage(
      "Satellite point list for editing line might be incomplete.",
      "warning",
    );
  }
  ...
  listItem.innerHTML = `
        <div class="edit-line-container">
            <input type="text" id="editLineName_${lineId}" value="${window.escapeHtml(
              currentRawName,
            )}" class="edit-line-name-input form-control" style="flex-grow:1; margin-right: 5px;">
  ...
  listItem
    .querySelector(".save-line-edit-button")
    .addEventListener("click", async (e) => {
      const newName = listItem
        .querySelector(".edit-line-name-input")
        .value.trim(); // raw
      ...
      if (!newSpId) {
        window.displayMessage(
          "Satellite Point ID cannot be empty for a line.",
          "error",
        );
        return;
      }
  ...
  listItem
    .querySelector(".cancel-line-edit-button")
    .addEventListener("click", () => {
      loadLines();
    });
}
...
async function executeUpdateLine(lineId, newName, newSatellitePointId) {
  // newName is raw
  try {
    const response = await window.fetch_put(`/planning/api/lines/${lineId}`, {
      name: newName,
      satellite_point_id: newSatellitePointId,
    });
    const responseData = await response
      .json()
      .catch(() => ({ message: "Invalid JSON response" }));
    ...
    if (!response.ok) {
      window.displayMessage(
        `Error updating line: ${responseData.message || response.status}`,
        "error",
      );
    } else {
      ...
      if (satellitePointSelect) {
        const selectedOption = Array.from(satellitePointSelect.options).find(
          (opt) => opt.value === String(newSatellitePointId),
        );
        if (selectedOption && selectedOption.textContent) {
          rawSpName = selectedOption.textContent; // This textContent should be raw or consistently escaped
        }
      }
      if (responseData.satellite_point_name) {
        // Prefer actual name from DB response
      ...
      if (rawSpName && rawSpName !== "Unknown Satellite Point") {
        window.displayMessage(
          `Line '${lineNameStr}' updated successfully for satellite point '${window.escapeHtml(
            rawSpName,
          )}'.`,
          "success",
        );
        ...
        window.displayMessage(
          `Line '${lineNameStr}' updated successfully.`,
          "success",
        );
      }
    }
    ...
    window.displayMessage(
      "Failed to update line. See console for details.",
      "error",
    );
  } finally {
    loadLines();
...
async function handleDeleteLine(event) {
  ...
  const cleanedLineName =
    typeof rawLineName === "string"
      ? rawLineName.replace(/\"/g, '"')
      : rawLineName;
  ...
  try {
    const response = await window.fetch_delete(`/planning/api/lines/${lineId}`);
    ...
    if (!response.ok) {
      ...
      window.displayMessage(
        `Error deleting line: ${errorData.message || response.status}`,
        "error",
      );
    } else {
      window.displayMessage(
        `Line '${window.escapeHtml(rawLineName)}' deleted successfully.`,
        "success",
      );
    }
  } catch (error) {
    window.displayMessage(
      "Failed to delete line. See console for details.",
      "error",
    );
  } finally {
    loadLines();
...
function populateSatellitePointDropdowns(satellitePoints) {
  const newLineSpSelect = document.getElementById(
    "newLineSatellitePointSelect",
  );
  ...
  if (techSpSelect) {
    ...
    if (
      window.GLOBAL_STATE &&
      window.GLOBAL_STATE.selectedTechnician &&
      window.GLOBAL_STATE.selectedTechnician.satellite_point_id
    ) {
      if (
        satellitePoints.some(
          (p) =>
            p.id.toString() ===
            window.GLOBAL_STATE.selectedTechnician.satellite_point_id.toString(),
        )
      ) {
        techSpSelect.value =
          window.GLOBAL_STATE.selectedTechnician.satellite_point_id;
        ...
        currentVal &&
        satellitePoints.some((p) => p.id.toString() === currentVal)
      ) {
...
async function handleTechnicianFormSubmit(event) {
  ...
  const satellitePointId = form.querySelector(
    'select[name="satellite_point_id"]',
  ).value;
  const technicianName = form.querySelector('input[name="name"]').value.trim();
  const technicianPhone = form
    .querySelector('input[name="phone"]')
    .value.trim();
  const technicianEmail = form
  ...
  try {
    ...
    if (technicianId) {
      ...
      response = await window.fetch_put(
        `/planning/api/technicians/${technicianId}`,
        technicianData,
      );
    } else {
      // Add new technician
      response = await window.fetch_post(
    ...
    if (!response.ok) {
      window.displayMessage(
        `Error saving technician details: ${
          responseData.message || response.status
        }`,
        "error",
      );
      ...
      window.displayMessage(
        `Technician details saved successfully.`,
        "success",
      );
    ...
    window.displayMessage(
      "Failed to save technician details. See console for details.",
      "error",
    );
  }
}
...
async function loadTechnicians() {
  try {
    const response = await window.fetch_get("/planning/api/technicians");
    if (!response.ok) {
      const errorData = await response
        .json()
        .catch(() => ({ message: "Invalid JSON response" }));
      window.displayMessage(
        `Error loading technicians: ${errorData.message || response.status}`,
        "error",
      );
      return;
    }
    ...
    window.displayMessage(
      "Failed to load technicians. See console for details.",
      "error",
    );
  }
}
...
function handleEditTechnician(event) {
  ...
  const satellitePointSelect = form.querySelector(
    'select[name="satellite_point_id"]',
  );

  // nameInput.value = rawTechnicianName; // Set initial raw name, then fetch full details

  fetch(`/planning/api/technicians/${technicianId}`)
    ...
    .catch((error) => {
      window.displayMessage(
        "Failed to load technician details. See console for details.",
        "error",
      );
    });
}
...
async function handleDeleteTechnician(event) {
  ...
  const cleanedTechName =
    typeof rawTechnicianName === "string"
      ? rawTechnicianName.replace(/\"/g, '"')
      : rawTechnicianName;

  if (
    !confirm(
      `Are you sure you want to delete technician "${cleanedTechName}" (ID: ${technicianId})?`,
    )
  ) {
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response
        ...
        .catch(() => ({ message: "Failed to parse error response." }));
      window.displayMessage(
        `Error deleting technician: ${errorData.message || response.status}`,
        "error",
      );
    } else {
      window.displayMessage(
        `Technician '${window.escapeHtml(
          rawTechnicianName,
        )}' deleted successfully.`,
        "success",
      );
    ...
    window.displayMessage(
      "Failed to delete technician. See console for details.",
      "error",
    );
  }
}
...
function initializeNewLineForm() {
  ...
  const newLineSatellitePointSelect = document.getElementById(
    "newLineSatellitePointSelect",
  );
  ...
  if (newLineNameInput && newLineSatellitePointSelect) {
    ...
    newLineNameInput.addEventListener("input", () => {
      newLineSatellitePointSelect.disabled =
        newLineNameInput.value.trim() === "";
    });
