...
async function handleAddSatellitePoint() {
    ...
    try {
        const response = await window.fetch_post('/planning/api/satellite_points', { name });
...
async function executeUpdateSatellitePoint(pointId, newName) { // newName is raw
    try {
        const response = await window.fetch_put(`/planning/api/satellite_points/${pointId}`, { name: newName });
...
async function handleDeleteSatellitePoint(event) {
    ...
    try {
        const response = await window.fetch_delete(`/planning/api/satellite_points/${pointId}`);
...
async function handleAddLine() {
    ...
    try {
        const response = await window.fetch_post('/planning/api/lines', { name, satellite_point_id: parseInt(satellitePointId) });
        const responseData = await response.json();
...
async function executeUpdateLine(lineId, newName, newSatellitePointId) { // newName is raw
    try {
        const response = await window.fetch_put(`/planning/api/lines/${lineId}`, { name: newName, satellite_point_id: newSatellitePointId });
        const responseData = await response.json().catch(() => ({ message: 'Invalid JSON response' }));
...
async function handleTechnicianFormSubmit(event) {
    ...
    try {
        ...
        if (technicianId) {
            ...
            response = await window.fetch_put(`/planning/api/technicians/${technicianId}`, technicianData);
        } else {
            // Add new technician
            response = await window.fetch_post('/planning/api/technicians', technicianData);
        }
        const responseData = await response.json().catch(() => ({ message: 'Invalid JSON response' }));
...
async function handleDeleteTechnician(event) {
    ...
    try {
        const response = await window.fetch_delete(`/planning/api/technicians/${technicianId}`);
