document.addEventListener("DOMContentLoaded", function () {
  ...
  const newTechnicianGroupNameInput = document.getElementById(
    "newTechnicianGroupName",
  );
  const technicianGroupListContainer = document.getElementById(
    "technicianGroupListContainer",
  );

  // Group Membership UI Elements
  const groupMembershipSelect = document.getElementById(
    "groupMembershipSelect",
  );
  const groupMembershipDetails = document.getElementById(
    "groupMembershipDetails",
  );
  const availableTechniciansList = document.getElementById(
    "availableTechniciansList",
  );
  const groupMembersList = document.getElementById("groupMembersList");
  const addTechnicianToGroupBtn = document.getElementById(
    "addTechnicianToGroupBtn",
  );
  const removeTechnicianFromGroupBtn = document.getElementById(
    "removeTechnicianFromGroupBtn",
  );
  const selectedGroupNameSpan = document.getElementById("selectedGroupName");
  const availableTechniciansSearch = document.getElementById(
    "availableTechniciansSearch",
  );
  const groupMembersSearch = document.getElementById("groupMembersSearch");
  const availableTechniciansCount = document.getElementById(
  ...
  function fetchAllTechnicians() {
    fetch("/planning/api/get_technician_mappings")
      .then((response) => response.json())
      .then((data) => {
        allTechnicians = Object.entries(data.technicians).map(
          ([name, techData]) => ({ id: techData.id, name: name }),
        );
      })
  ...
  function fetchTechnicianGroups() {
    fetch("/planning/api/technician_groups") // Corrected endpoint
      .then((response) => response.json())
      .then((data) => {
        allGroups = data;
  ...
  function renderTechnicianGroups(groups) {
    ...
    if (groups.length === 0) {
      technicianGroupListContainer.innerHTML =
        "<p>No technician groups found.</p>";
      return;
    }
  ...
  function populateGroupMembershipSelect(groups) {
    groupMembershipSelect.innerHTML =
  ...
  technicianGroupForm.addEventListener("submit", function (e) {
    ...
    fetch("/planning/api/technician_groups", {
      // Corrected endpoint
      method: "POST",
      headers: {
        ...
        if (response.ok) {
          if (result.id) {
            ...
            window.displayMessage(
              `Group '${result.name}' created successfully.`,
              "success",
            );
          } else {
            // This case might indicate a successful response but unexpected data structure
            window.displayMessage(
              result.message || "Error creating group: Unexpected response.",
              "error",
            );
          }
        } else {
          ...
          window.displayMessage(
            result.message ||
              `Error creating group: Server error ${response.status}`,
            "error",
          );
        }
      })
      .catch((error) => {
        ...
        window.displayMessage(
          `Failed to add technician group. Network error or invalid response.`,
          "error",
        );
      });
  });
  ...
  technicianGroupListContainer.addEventListener("click", function (e) {
    ...
    if (editBtn) {
      ...
      if (confirm("Are you sure you want to delete this group?")) {
        fetch(`/planning/api/technician_groups/${groupId}`, {
          // Corrected endpoint
          method: "DELETE",
          headers: {
            "X-CSRFToken": csrfToken,
          ...
          .then((response) => {
            if (response.ok) {
              ...
              response
                .json()
                .then((data) => alert(data.message || "Error deleting group"));
            }
          })
          .catch((error) =>
            console.error("Error deleting technician group:", error),
          );
      }
    } else if (saveBtn) {
        ...
        fetch(`/planning/api/technician_groups/${groupId}`, {
          ...
          .then((response) => {
            if (response.ok) {
              ...
              response
                .json()
                .then((data) => alert(data.message || "Error updating group"));
            }
          })
          .catch((error) =>
            console.error("Error updating technician group:", error),
          );
      }
    } else if (cancelBtn) {
      fetchTechnicianGroups();
  ...
  function loadGroupMembers(groupId) {
    fetch(`/planning/api/technician_groups/${groupId}/members`) // Corrected endpoint
      .then((response) => response.json())
      .then((members) => {
        const memberIds = new Set(members.map((m) => m.id));
        const availableTechnicians = allTechnicians.filter(
          (t) => !memberIds.has(t.id),
        );
  ...
  function renderMembershipLists(available, members) {
    renderList(
      availableTechniciansList,
      available,
      availableTechniciansSearch.value,
      availableTechniciansCount,
    );
    renderList(
      groupMembersList,
  ...
  function updateButtonStates() {
    const selectedAvailable =
      availableTechniciansList.querySelectorAll(".technician-card.selected")
        .length > 0;
    addTechnicianToGroupBtn.disabled = !selectedAvailable;

    const selectedMembers =
      groupMembersList.querySelectorAll(".technician-card.selected").length > 0;
  ...
  addTechnicianToGroupBtn.addEventListener("click", () =>
    moveTechnicians(availableTechniciansList, true),
  );
  removeTechnicianFromGroupBtn.addEventListener("click", () =>
    moveTechnicians(groupMembersList, false),
  );
  ...
  function moveTechnicians(sourceList, isAdding) {
    ...
    const selectedItems = Array.from(
      sourceList.querySelectorAll("div.technician-card.selected"),
    );
    ...
    const promises = technicianIds.map((technicianId) => {
      return fetch("/planning/api/technician_groups/members", {
        // Corrected endpoint
