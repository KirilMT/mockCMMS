...
def register_template_filters(app):
    ...
    def maintenance_url(task_name):
        try:
            ...
            endpoint = config._config['internal_system']['endpoints']['maintenance_grid']
            
            params = [
                'status=NEW', 'status=IN_PRG', 'status=WT_PT', 'status=SCH_HD',
                'status=RD_PICKUP', 'status=RDY_TO_ASSG', 'status=IN_PLAN', 
                'status=IN_BUILD', 'status=ASSGN', 'assetViewIds=2642',
