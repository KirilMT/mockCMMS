...
**Goal:** Retire legacy `planning` pieces that are no longer needed after integration, and address terminology confusion.
...
- [ ] 6.1. **Terminology & Model Renaming** 🆕 🔴 **HIGH PRIORITY - USER REQUESTED**
  - [ ] 6.1.1. **Rename "Schedule" to "MaintenancePlan"** to avoid confusion with recurring maintenance schedules
    ...
    - **Affected Files:**
      - `apps/planning/src/services/planning_models.py` - Model definition
      - `apps/planning/src/services/planning_engine.py` - Engine logic
      - `apps/planning/src/routes/planning.py` - All routes
      - `apps/planning/src/templates/planning/*.html` - All templates
      - `apps/planning/tests/*.py` - All test files
...
- [ ] 6.2. Remove Excel-based workflow components
  - [ ] 6.2.1. Delete or archive Excel extraction scripts and services in `apps/planning`.
  ...
  - [ ] 6.2.2. Remove the Manage Mappings page and its logic.
    - Routes to remove from `planning.py`
...
- [ ] 6.4. Update documentation
  - [ ] 6.4.1. Update `apps/planning/README.md` to reflect integrated role and new architecture.
...
- [ ] 6.4.5. **Clean Up Deprecated Test Files** 🆕 (November 20, 2025)
  ...
  - **Files to Handle:**
    - [ ] **DELETE: `test_core.py`** - 22 deprecated tests
      ...
      - From old standalone planning architecture
...
- [ ] 6.5. **Security Cleanup** 🔴 **CRITICAL - BEFORE PRODUCTION** (Based on PROJECT_ISSUES.md findings)
  ...
  - [x] 6.5.4. **Security Audit of Planning Module JavaScript** ✅ **COMPLETE - November 20, 2025**
    ...
    - **Files Audited:**
      - ✅ `apps/planning/src/static/js/planning-gantt-custom.js` (450 lines)
      - ✅ `apps/planning/src/static/js/planning-gantt.js` (partial)
    - **Security Findings:**
