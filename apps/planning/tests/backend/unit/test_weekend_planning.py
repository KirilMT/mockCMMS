...
from apps.planning.src.services.planning_engine import PlanningEngine  # noqa: E402
...
class TestWeekendPlanning:
    ...
    def test_shift_intersection_friday_night(self, engine):
        """Test that Friday maintenance shift starts at 22:00 (intersection of Late
        Shift and Window Start)."""
        # Friday
        start_date = datetime(
            2023, 11, 24, 22, 0
        )  # Nov 24 2023 Friday 22:00 - Weekend window start
        end_date = datetime(2023, 11, 26, 22, 0)  # Sunday 22:00

        shifts = engine._get_weekend_shifts(start_date, end_date)
        ...
        assert first_shift.is_overnight is True
    ...
    def test_shift_intersection_saturday_full(self, engine):
        ...
        assert late.is_overnight is True
    ...
    def test_shift_intersection_sunday_end(self, engine):
        ...
        if len(sun_shifts) > 1:
            ...
            assert late.is_overnight is False  # Ends same day
    ...
    def test_assign_task_to_shift(self, engine):
        ...
        tech.id = 1
        tech.name = "John Doe"
        tech.username = "John Doe"
        tech.skills = []

        workloads = {
            1: MagicMock(total_available_minutes=1000, total_assigned_minutes=0)
        }
