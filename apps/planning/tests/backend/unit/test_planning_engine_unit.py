...
def mock_all_models():
    ...
    with (
        ...
        m_team.query.filter.return_value.order_by.return_value.all.return_value = []
        m_user.query.filter.return_value.order_by.return_value.all.return_value = []
        m_pt.query.filter.return_value.order_by.return_value.all.return_value = []
...
class TestPlanningEngine:
        ...
        with (
            patch.object(
                ...
                return_value=[
                    (
                        MagicMock(id=1),
                        MagicMock(id=1, priority="High", required_skills=[]),
                    )
                ],
            ),
    ...
    def test_generate_plan_complex(
        ...
        tech = MagicMock(id=101, team=team)
        skill1 = MagicMock()
