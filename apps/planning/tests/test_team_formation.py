...
def app_context_team_tests():
    ...
    with app.app_context():
        ...
        alice = Technician(id=1, name="Alice", availability_status="Available")
        alice.skills = [
            TechnicianSkill(
                technician_id=1, skill_id=1, skill_level=5
            ),  # Electrical - Expert
            TechnicianSkill(
                technician_id=1, skill_id=2, skill_level=4
            ),  # Mechanical - Advanced
        ]

        # Mid-level specialist
        bob = Technician(id=2, name="Bob", availability_status="Available")
        bob.skills = [
            TechnicianSkill(
                technician_id=2, skill_id=3, skill_level=4
            ),  # PLC - Advanced
            TechnicianSkill(
                technician_id=2, skill_id=1, skill_level=3
            ),  # Electrical - Intermediate
        ]

        # Junior technician with limited skills
        charlie = Technician(id=3, name="Charlie", availability_status="Available")
        charlie.skills = [
            TechnicianSkill(
                technician_id=3, skill_id=2, skill_level=2
            ),  # Mechanical - Beginner
        ]

        # Senior specialist
        diana = Technician(id=4, name="Diana", availability_status="Available")
        diana.skills = [
            TechnicianSkill(
                technician_id=4, skill_id=4, skill_level=5
            ),  # Robotics - Expert
            TechnicianSkill(
                technician_id=4, skill_id=1, skill_level=4
            ),  # Electrical - Advanced
        ]

        # Mid-level generalist
        eve = Technician(id=5, name="Eve", availability_status="Available")
        eve.skills = [
...
def test_two_person_team_skill_coverage(app_context_team_tests):
    ...
    with app_context_team_tests.app_context():
        ...
        task = PlanningTask(maintenance_order_id=1, schedule_id=1, status="Unplanned")
        db.session.add(task)
        db.session.commit()
...
def test_experience_balancing_in_team(app_context_team_tests):
    ...
    with app_context_team_tests.app_context():
        ...
        task = PlanningTask(maintenance_order_id=2, schedule_id=1, status="Unplanned")
        db.session.add(task)
        db.session.commit()
        ...
        team_avg_levels = []
        for tech in team:
            electrical_skill = next(
                (ts for ts in tech.skills if ts.skill.name == "Electrical"), None
            )
            if electrical_skill:
        ...
        assert any(
            level >= 4 for level in team_avg_levels
...
def test_three_person_multi_skill_team(app_context_team_tests):
    ...
    with app_context_team_tests.app_context():
        ...
        task = PlanningTask(maintenance_order_id=3, schedule_id=1, status="Unplanned")
        db.session.add(task)
        db.session.commit()
...
def test_team_cannot_be_formed_insufficient_skills(app_context_team_tests):
    ...
    with app_context_team_tests.app_context():
        ...
        task = PlanningTask(maintenance_order_id=4, schedule_id=1, status="Unplanned")
        db.session.add(task)
        db.session.commit()
        ...
        assert (
            "Underwater Welding" in unassigned.reason_detail
        )  # Skill name in detail message
...
def test_single_person_task_selects_best_candidate(app_context_team_tests):
    ...
    with app_context_team_tests.app_context():
        ...
        task = PlanningTask(maintenance_order_id=5, schedule_id=1, status="Unplanned")
        db.session.add(task)
        db.session.commit()
...
def test_workload_distribution_across_team_tasks(app_context_team_tests):
    ...
    with app_context_team_tests.app_context():
        ...
        for i in range(1, 4):
            ...
            task = PlanningTask(
                maintenance_order_id=i, schedule_id=1, status="Unplanned"
            )
            db.session.add(task)
        ...
        for assigned in result.assigned_tasks:
            for tech_id in assigned.assigned_technician_ids:
                tech_assignment_counts[tech_id] = (
                    tech_assignment_counts.get(tech_id, 0) + 1
                )
