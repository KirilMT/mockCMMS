...
This document tracks all identified bugs in the **Reporting** application (`apps/reporting`).
Each bug includes:
...
> [!NOTE]
> **Document Relationship:** This document tracks bugs specific to the Reporting app.
> For bugs in the core mockCMMS application, see `docs/bug_tracking.md`.
> For Reporting feature planning, see `apps/reporting/docs/reporting_roadmap.md`.
...
- User is on a Report detail page (e.g., `/reporting/shift/5`).
...
- Expected: button should read **"Back to Report"** and navigate back to `/reporting/shift/5`.
...
- When arriving at any entity detail page **from a Report**, the back button should:
  ...
  - Navigate back to the exact Report page the user came from (e.g., `/reporting/shift/5`).
...
4. **Label logic:** Detect the origin type from the `return_to` URL to set a meaningful button label
   (e.g., URL contains `/reporting/` → label = "Back to Report").
5. **Security:** URL-encode and validate `return_to` to prevent open redirect vulnerabilities.
   Only allow internal relative paths.

**Affected Files (Likely):**

- `apps/reporting/src/templates/report_detail.html` — Add `?return_to=` to all entity links
...
_This document tracks bugs for the Reporting (`apps/reporting`) application._
