...
class TestShiftReportRoutesIntegration:
    ...
    @patch("apps.reporting.src.routes.shift_report.render_template")
    def test_shift_report_get_default_params(self, mock_render, client, auth_headers):
        ...
        response = client.get("/reporting/shift/", headers=auth_headers)
    ...
    def test_shift_report_get_with_custom_params(
        ...
        mock_render.return_value = "Mocked Shift Report"
        response = client.get(
            "/reporting/shift/?date=2026-02-08&shift=Night", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_shift_report_get_with_data(self, mock_render, app, client, auth_headers):
        ...
        response = client.get(
            "/reporting/shift/?date=2026-02-08&shift=Morning", headers=auth_headers
        )
        assert response.status_code == 200
        mock_render.assert_called_once()
    ...
    def test_shift_report_different_shifts(self, mock_render, client, auth_headers):
        ...
        for shift in ["Morning", "Afternoon", "Night"]:
            response = client.get(
                f"/reporting/shift/?date=2026-02-08&shift={shift}", headers=auth_headers
            )
            assert response.status_code == 200
...
class TestWeekendReportRoutesIntegration:
    ...
    @patch("apps.reporting.src.routes.weekend_report.render_template")
    def test_weekend_report_get_default_params(self, mock_render, client, auth_headers):
        ...
        response = client.get("/reporting/weekend/", headers=auth_headers)
        assert response.status_code == 200
        mock_render.assert_called_once()
    ...
    def test_weekend_report_get_with_custom_dates(
        ...
        mock_render.return_value = "Mocked Weekend Report"
        response = client.get(
            "/reporting/weekend/?start_date=2026-02-08&end_date=2026-02-09",
    ...
    def test_weekend_report_single_day_range(self, mock_render, client, auth_headers):
        ...
        mock_render.return_value = "Mocked Weekend Report"
        response = client.get(
            "/reporting/weekend/?start_date=2026-02-08&end_date=2026-02-08",
            headers=auth_headers,
        )
        assert response.status_code == 200
    ...
    def test_weekend_report_multi_day_range(self, mock_render, client, auth_headers):
        ...
        mock_render.return_value = "Mocked Weekend Report"
        response = client.get(
            "/reporting/weekend/?start_date=2026-02-08&end_date=2026-02-10",
            headers=auth_headers,
        )
        assert response.status_code == 200
...
class TestReportExportRoutes:
    """Test report export functionality for shift and weekend reporting."""

    def test_shift_export_requires_post(self, client, auth_headers):
        """Test shift export endpoint requires POST method."""
        response = client.get("/reporting/shift/export", headers=auth_headers)
    ...
    def test_weekend_export_requires_post(self, client, auth_headers):
        ...
        response = client.get("/reporting/weekend/export", headers=auth_headers)
        assert response.status_code == 405  # Method Not Allowed

    @patch("apps.reporting.src.routes.shift_report.send_file")
    @patch("apps.reporting.src.routes.shift_report.ReportGenerator")
    @patch("apps.reporting.src.routes.shift_report.DataAggregator")
    ...
    @patch("apps.reporting.src.routes.weekend_report.send_file")
    @patch("apps.reporting.src.routes.weekend_report.ReportGenerator")
    @patch("apps.reporting.src.routes.weekend_report.DataAggregator")
...
class TestReportRoutesEdgeCases:
    ...
    def test_shift_report_with_future_date(self, mock_render, client, auth_headers):
        ...
        mock_render.return_value = "Mocked Shift Report"
        response = client.get(
            "/reporting/shift/?date=2030-01-01&shift=Morning", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_shift_report_with_past_date(self, mock_render, client, auth_headers):
        ...
        mock_render.return_value = "Mocked Shift Report"
        response = client.get(
            "/reporting/shift/?date=2020-01-01&shift=Night", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_weekend_report_with_future_dates(self, mock_render, client, auth_headers):
        ...
        mock_render.return_value = "Mocked Weekend Report"
        response = client.get(
            "/reporting/weekend/?start_date=2030-01-01&end_date=2030-01-02",
            headers=auth_headers,
        )
        assert response.status_code == 200
    ...
    def test_weekend_report_with_past_dates(self, mock_render, client, auth_headers):
        ...
        mock_render.return_value = "Mocked Weekend Report"
        response = client.get(
            "/reporting/weekend/?start_date=2020-01-01&end_date=2020-01-02",
            headers=auth_headers,
        )
        assert response.status_code == 200
