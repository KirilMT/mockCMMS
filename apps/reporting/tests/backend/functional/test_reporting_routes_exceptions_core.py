...
from apps.reporting.src.routes.reporting import (
    _build_display_title,
    _resolve_shift_team_id,
    _safe_json_for_template,
    _sync_file_reporting,
    export_report,
    generate_report,
    report_detail,
    reporting,
...
class TestReportingExceptions:
    """Test exception handling and edge cases in reporting routes."""

    def test_reporting_index_table_creation_exception(self, app):
        """Test exception during table creation in reporting index."""
        with app.app_context(), app.test_request_context():
            ...
            with patch(
                ...
                assert "reporting" in str(response)

    def test_reporting_index_user_lookup_exception(self, app):
        """Test exception during user lookup in reporting index."""
        with app.app_context(), app.test_request_context():
            session["user_id"] = 1
    ...
    def test_reporting_index_general_exception(self, app):
        """Test general exception in reporting index."""
        with app.app_context(), app.test_request_context():
            session["user_id"] = 1
            with patch(
                "apps.reporting.src.models.Report.query",
                side_effect=Exception("Query failed"),
            ):
                response = reporting()
                # Should return empty reporting list
                assert "reporting" in str(response)
    ...
    def test_sync_file_reporting_skips_existing_and_handles_invalid_file(self, app):
        with app.app_context():
            with (
                patch(
                ...
                patch(
                    "apps.reporting.src.routes.reporting.os.path.exists",
                    return_value=True,
                ),
                patch(
                    "apps.reporting.src.routes.reporting.os.listdir",
                    return_value=["existing.json", "invalid.json"],
                ),
                patch(
                    "apps.reporting.src.routes.reporting.open", create=True
                ) as mock_open,
                patch(
                    "apps.reporting.src.routes.reporting.report_models.Report.query"
                ) as mock_query,
                patch("apps.reporting.src.routes.reporting.db.session") as mock_session,
            ):
                mock_rg.return_value.reporting_dir = "C:/tmp/reporting"
                mock_query.with_entities.return_value.all.return_value = [
                    ("C:/tmp/reporting/existing.json",)
                ]
        ...
        with app.app_context():
            with app.test_request_context(
                ...
                with (
                    ...
                    patch(
                        "apps.reporting.src.routes.reporting._resolve_shift_team_id",
                        return_value=("5", "Team Night"),
                    ),
                    patch(
                        "apps.reporting.src.routes.reporting.db.session"
                    ) as mock_session,
                ):
...
class TestGenerateReportCoverage:
    ...
    def test_resolve_shift_team_id_success(self, app):
        with app.app_context():
            ...
            with (
                patch("apps.reporting.src.routes.reporting.Team") as mock_team,
                patch(
                    "apps.reporting.src.routes.reporting.get_shift_teams",
    ...
    def test_generate_shift_report_hits_fallback_handover_branch(self, app):
        with (
            ...
            with (
                ...
                patch(
                    "apps.reporting.src.routes.reporting.db.session.add",
                    side_effect=capture_add,
                ),
                patch("apps.reporting.src.routes.reporting.db.session.commit"),
            ):
                mock_agg.return_value.get_aggregated_shift_data.return_value = {}
                response = generate_report()
        ...
        with app.app_context():
            ...
            with app.test_request_context():
                ...
                with (
                    ...
                    patch(
                        "apps.reporting.src.routes.reporting.render_template"
                    ) as mock_render,
                ):
        ...
        with app.app_context():
            ...
            with app.test_request_context():
                ...
                with (
                    ...
                    patch(
                        "apps.reporting.src.routes.reporting.os.makedirs"
                    ) as mock_makedirs,
                    patch("apps.reporting.src.routes.reporting.open", create=True),
                    patch("apps.reporting.src.routes.reporting.json.dump") as mock_dump,
                ):
        ...
        with app.app_context():
            ...
            with app.test_request_context():
                ...
                with (
                    ...
                    patch(
                        "apps.reporting.src.routes.reporting.rg_service."
