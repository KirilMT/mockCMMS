/**
 * Unit tests for apps/reporting/src/static/js/report-interactions.js
...
describe("report-interactions utilities", () => {
  ...
  describe("copy and update helpers", () => {
    ...
    test("sendUpdate posts to report endpoint and handles success", async () => {
      window.history.pushState({}, "", "/reporting/15");
      ...
      expect(url).toBe("/reporting/15/update");
    });

    test("sendUpdate returns early for invalid URL", () => {
      window.history.pushState({}, "", "/reporting/not-a-number");
