...
def _sync_file_reports():
    """Scan the reports directory for JSON files and ensure they exist in the DB."""
    try:
        report_dir = rg_service.ReportGenerator().reports_dir
        ...
        if new_reports:
            db.session.add_all(new_reports)
            db.session.commit()
            current_app.logger.info(
                f"Synced {len(new_reports)} file-based reports to DB."
            )

    except Exception as e:
        current_app.logger.error(f"Error syncing file reports: {e}")
...
def reports():
    try:
        ...
        # Sync legacy file-based reports to DB
        _sync_file_reports()
        ...
        return render_template(
            "reports.html", reports=reports_data, user_data=user_data, teams=teams
        )
    except Exception as e:
        current_app.logger.error(f"Error loading reports: {e}", exc_info=True)
        # Fallback if table creation fails or other DB issues
        return render_template("reports.html", reports=[], user_data={}, teams=[])
...
def report_detail(report_id):
    ...
    # Keep backward-compatible alias for shift reports
    ...
    # Weekend seeded reports have handover data nested in report_info
    ...
    def linkify_spare_parts(text):
        ...
        terms = sorted(spare_part_link_map.keys(), key=len, reverse=True)
    ...
    default_config_data = {
        "metadata_totals": {
            ...
            "mds_total": 15,
            "description": (
                "Total numbers for each equipment or technology type in shift reports. "
...
def update_report_data(report_id):
    ...
    try:
        ...
        if section == "header":
            ...
            # Backward compatibility for shift reports
            if report_type == "shift_report":
                current_data["shift_info"] = current_data["report_info"]

            # Weekend reports also store shift at top level
            ...
            # Backward compatibility for non-weekend reports
            if report_type != "weekend_report":
