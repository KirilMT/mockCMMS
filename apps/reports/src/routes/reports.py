...
def reports():
    try:
        ...
        check_query = "SELECT name FROM sqlite_master WHERE type='table' AND name='reports'"
        ...
        if not table_exists:
            ...
            for row in result:
                reports_data.append({
                    ...
                    'format': row.format,
                    'generated_on': row.generated_on.isoformat() if row.generated_on else None,
        ...
        flash(f'Reports feature is not yet available. Database setup required.', 'warning')
...
def generate_report():
    if request.method == 'POST':
        flash('Report generation is not yet available. Database setup required.', 'warning')
