...
class DataAggregator:
    def get_weekend_tasks(self, start_date, end_date):
        ...
        tasks = MaintenanceOrder.query.filter(
            MaintenanceOrder.due_date >= start,
            MaintenanceOrder.due_date < end
        ).all()
    ...
    def get_incidents(self, filters=None):
        ...
        if filters:
            ...
            if filters.get('start_date'):
                query = query.filter(Incident.timestamp >= datetime.strptime(filters['start_date'], "%Y-%m-%d"))
            if filters.get('end_date'):
                query = query.filter(Incident.timestamp <= datetime.strptime(filters['end_date'], "%Y-%m-%d"))
