...
class TestAuthentication:
    ...
    def test_login_success(self, client, admin_user):
        ...
        assert '/assets' in response.location or '/' in response.location, "Should redirect to index/assets"

        # Check session was created
        with client.session_transaction() as sess:
            assert 'user_id' in sess, "Session should contain user_id"
            assert sess['user_id'] == admin_user.id, "Session user_id should match admin user"
    ...
    def test_login_invalid_credentials(self, client, admin_user):
        ...
        with client.session_transaction() as sess:
            assert 'user_id' not in sess, "Session should NOT contain user_id after failed login"
    ...
    def test_logout(self, client, admin_user):
        ...
        with client.session_transaction() as sess:
            assert 'user_id' not in sess, "Session should NOT contain user_id after logout"
            assert 'username' not in sess, "Session should NOT contain username after logout"
    ...
    def test_admin_only_route_blocks_technician(self, client, admin_user, technician_user):
        """
    ...
    def test_password_hashing(self, app, admin_user):
        ...
        with app.app_context():
            ...
            assert user.password_hash != 'admin123', "Password should be hashed, not plain text"
            assert len(user.password_hash) > 20, "Password hash should be substantial length"
            assert 'pbkdf2:sha256' in user.password_hash or '$' in user.password_hash, \
                "Should use secure hashing algorithm"

            # Verify correct password validates
            assert user.check_password('admin123') is True, "Correct password should validate"

            # Verify incorrect password fails
            assert user.check_password('wrongpassword') is False, "Incorrect password should fail"
