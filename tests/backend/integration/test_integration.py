...
class TestIntegration:
    ...
    def test_complete_maintenance_workflow(self, client, app, admin_user):
        ...
        response = client.post(
            f"/maintenance_orders/{mo_id}/edit",
            data={
                "asset_id": str(asset_id),
                "description": "Integration test MO",
                "order_type": "reactive",
    ...
    def test_user_registration_to_assignment(self, client, app, admin_user):
        ...
        response = client.post(
            "/login",
            data={"username": "newtech", "password": "newtech123"},
            follow_redirects=True,
        )
        assert response.status_code == 200
    ...
    def test_shift_team_rotation_workflow(self, client, app, admin_user):
        ...
        with app.app_context():
            ...
            user1 = User(
                username="shift_tech1", email="st1@test.com", team_id=team_a_id
            )
            user1.set_password("pass123")
            user1.roles.append(tech_role)

            user2 = User(
                username="shift_tech2", email="st2@test.com", team_id=team_b_id
            )
    ...
    def test_cascade_relationships(self, client, app, admin_user):
        ...
        with app.app_context():
            asset = Asset(asset_code="CASCADE-001", name="Cascade Test Asset")
            db.session.add(asset)
            db.session.flush()
    ...
    def test_multi_user_concurrent_access(
        ...
        with app.app_context():
            asset = Asset(asset_code="CONCURRENT-001", name="Concurrent Access Test")
            db.session.add(asset)
            db.session.commit()
            asset_id = asset.id
    ...
    def test_complete_user_workflow_enhanced(self, client, app, admin_user):
        ...
        response = client.post(
            "/login",
            data={"username": "workflow_user", "password": "pass123"},
            follow_redirects=True,
        )
        assert response.status_code == 200
