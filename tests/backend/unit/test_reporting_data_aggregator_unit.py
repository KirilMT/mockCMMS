...
def test_get_shift_incidents_missing_asset(aggregator):
    ...
    with patch(
        "apps.reporting.src.services.data_aggregator.MaintenanceOrder"
    ) as MockMO:
...
def test_get_weekend_tasks(aggregator):
    ...
    with patch(
        ...
        with patch(
            "apps.reporting.src.services.data_aggregator.datetime"
        ) as mock_dt_cls:
            ...
            # Also timedelta
            with patch("apps.reporting.src.services.data_aggregator.timedelta"):
                # Setup filter return
                mock_task = MagicMock()
...
def test_get_shift_data(aggregator):
    ...
    with (
        patch("apps.reporting.src.services.data_aggregator.ShiftUtils") as MockUtils,
        patch("apps.reporting.src.services.data_aggregator.MaintenanceOrder") as MockMO,
    ):
...
def test_get_aggregated_shift_data_handles_naive_completion_datetime(aggregator):
    ...
    with (
        patch("apps.reporting.src.services.data_aggregator.ShiftUtils") as mock_utils,
        patch(
            "apps.reporting.src.services.data_aggregator.MaintenanceOrder"
        ) as mock_mo_cls,
    ):
...
def test_extract_assignee_ids_supports_legacy_user_and_team_tokens(aggregator):
    ...
    with (
        patch("apps.reporting.src.services.data_aggregator.User") as mock_user,
        patch("apps.reporting.src.services.data_aggregator.Team") as mock_team,
    ):
...
def test_get_team_user_ids_valid_and_invalid(aggregator):
    with patch("apps.reporting.src.services.data_aggregator.User") as mock_user:
        mock_user.query.filter_by.return_value.all.return_value = [
            MagicMock(id=1),
            MagicMock(id=2),
        ]
...
def test_get_aggregated_shift_data_splits_engineering_and_flux(aggregator, app):
    ...
    with app.app_context():
        with (
            patch(
                "apps.reporting.src.services.data_aggregator.ShiftUtils"
            ) as mock_utils,
            patch(
                "apps.reporting.src.services.data_aggregator.MaintenanceOrder",
                new=_DummyMO,
