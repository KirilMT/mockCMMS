...
def test_view_schedule_passes_config(app):
    with app.app_context():
        with patch('apps.planning.src.routes.planning.Schedule') as MockSchedule:
            with patch('apps.planning.src.routes.planning.render_template') as mock_render:
                with patch('apps.planning.src.routes.planning.load_shift_config') as mock_load_config:
                    with patch('apps.planning.src.routes.planning.PlanningTask') as MockPlanningTask:
                        with patch('apps.planning.src.routes.planning.MaintenanceOrder') as MockMO:

                            # Setup mocks
                            mock_schedule = MagicMock()
                            MockSchedule.query.get_or_404.return_value = mock_schedule

                            mock_config = {"shift_durations": {"shift_break": 10, "weekend": 20}}
