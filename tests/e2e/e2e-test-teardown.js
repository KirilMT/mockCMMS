...
async function globalTeardown(config) {
    ...
    if (fs.existsSync(TEST_DB_PATH)) {
        try {
            ...
            console.log(`⚠️  Could not remove test database (likely locked): ${error.message}`);
    ...
    if (fs.existsSync(INSTANCE_DIR)) {
        try {
            ...
            if (files.length === 0) {
                ...
                console.log(`   Instance directory not empty, keeping: ${files.join(', ')}`);
            }
        } catch (error) {
            console.log(`⚠️  Could not check/remove instance directory: ${error.message}`);
        }
    }
