...
describe("TableSidebar", () => {
  ...
  beforeEach(() => {
    ...
    AdvancedTable.prototype.showButtonLoading = jest.fn(() => ({
      restore: jest.fn(),
    }));
  ...
  test("TS-1.10: test_saveView_calls_api", async () => {
    ...
    expect(table.fetchWithRetry).toHaveBeenCalledWith(
      "/api/table-config/" + table.pageName,
  ...
  test("TS-1.11: test_deleteView_calls_api", async () => {
    ...
    global.showDeleteConfirm.mockImplementation(
      (entity, message, onConfirm) => {
        if (onConfirm) onConfirm();
      },
    );
    table.showButtonLoading = jest.fn(() => ({ restore: jest.fn() }));
  ...
  test("TS-2.6: test_addFilterRow_hides_empty_message", () => {
    ...
    filterRows.innerHTML =
      '<p id="noFiltersMessage" style="display: block;">No applied filters</p>';
    sidebar.addFilterRow();
    const msg = document.getElementById("noFiltersMessage");
  ...
  test("TS-2.9: test_restoreFilterUI_loads_applied_filters", () => {
    table.filters = [
      { column: "name", operator: "contains", value: "Restored" },
    ];
  ...
  test("TS-5.1: createFilterRow creates filter inputs", () => {
    ...
    document.body.innerHTML =
      '<div id="test-container"><div id="filterRows"></div></div>';
    sidebar.addFilterRow();
    const rows = document.querySelectorAll(".filter-row-sidebar");
  ...
  describe("Feature: View Management & Input Logic", () => {
    ...
    test("TS-8.1: Update button truncates long view names", () => {
      document.body.innerHTML =
        '<div id="test-container"><button id="updateViewBtn"></button><div id="savedViewsList"></div></div>';
      table.lastLoadedConfigId = 1;
      table.savedConfigs = [
        {
          id: 1,
          config_name: "A Very Long View Name That Should Be Truncated",
        },
      ];
    ...
    test("TS-8.3: Toggle Default View branches", () => {
      // ... existing test ...
      document.body.innerHTML =
        '<div id="test-container"><div id="savedViewsList"></div></div>';
  ...
  test("attachEventListeners matches toggle buttons", () => {
    ...
    const spyToggle = jest
      .spyOn(sidebar, "toggleSidebar")
      .mockImplementation(() => {});
    toggleBtn.click();
    expect(spyToggle).toHaveBeenCalled();

    const header = div.querySelector(".section-header");
    const spySection = jest
      .spyOn(sidebar, "toggleSection")
      .mockImplementation(() => {});
    header.click();
