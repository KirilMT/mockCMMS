...
describe("TableConfig Module", () => {
  ...
  test("TG-1.1: test_saveView_creates_named_configuration", async () => {
    ...
    const loadConfigSpy = jest
      .spyOn(table, "loadConfiguration")
      .mockImplementation(() => {});

    await table.saveConfiguration();

    expect(global.fetch).toHaveBeenCalledWith(
      "/api/table-config/test-page",
  ...
  test("TG-1.4: test_setDefaultView_persists_preference", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      "/api/table-config/test-page/10/default",
      expect.objectContaining({
        method: "POST",
      }),
    );
  ...
  test("TG-2.8: test_deleteConfiguration_handles_error", async () => {
    ...
    expect(ToastNotification.error).toHaveBeenCalledWith(
  ...
  test("TG-3.3: test_applyConfiguration_is_callable", () => {
    ...
    table.saveTableState = jest.fn();
    table.sidebar = {
      loadExistingFilters: jest.fn(),
      populateColumns: jest.fn(),
      populateSavedViews: jest.fn(),
    };
  ...
  test("TG-4.2: test_loadConfiguration_http_500", async () => {
    ...
    expect(ToastNotification.warning).toHaveBeenCalledWith(
      "Could not load saved configurations",
    );
  });
  ...
  test("TG-4.3: test_loadConfiguration_max_retries", async () => {
    table.fetchWithRetry = jest
      .fn()
      .mockRejectedValue(new Error("Max retries exceeded"));
    console.error = jest.fn();
  ...
  test("TG-4.5: saveConfiguration shows error when data.success is false", async () => {
    ...
    expect(ToastNotification.error).toHaveBeenCalledWith(
      "Failed to save configuration: Duplicate name",
    );
  });
});
