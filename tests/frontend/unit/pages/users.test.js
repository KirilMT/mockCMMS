...
describe("Users Page - Render Functions", () => {
    ...
    describe("Initialization", () => {
        test("initializes table with data", () => {
            document.body.innerHTML = '<div id="users-data">[{"id":1, "username":"test"}]</div>';
            initUsersTable();
        ...
        test("does not initialize if data element missing", () => {
            ...
            initUsersTable();
            expect(global.initAdvancedTable).not.toHaveBeenCalledWith("usersTable", expect.any(Array), expect.any(Array), expect.any(Number));
        });
    });
    ...
    describe("renderUsername", () => {
        test("returns username without warning for non-technician", () => {
            const result = renderUsername("john_doe", { is_technician: false, team_id: null });
            expect(result).toBe("john_doe");
        });

        test("returns username without warning for technician with team", () => {
            const result = renderUsername("jane_doe", { is_technician: true, team_id: 1 });
            expect(result).toBe("jane_doe");
        });

        test("returns username with warning for technician without team", () => {
            const result = renderUsername("bob_tech", { is_technician: true, team_id: null });
        ...
        test("returns username with warning when team_id is 0 (falsy)", () => {
            const result = renderUsername("tech_user", { is_technician: true, team_id: 0 });
            expect(result).toContain("⚠️");
        });

        test("returns username without warning when is_technician is false and team_id is null", () => {
            const result = renderUsername("regular_user", { is_technician: false, team_id: null });
    ...
    describe("renderAvailability", () => {
        ...
        test("returns raw value for unknown status", () => {
            const result = renderAvailability("Custom Status", { is_technician: true });
