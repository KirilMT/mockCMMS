...
describe("Maintenance Order Detail Page", () => {
  ...
  beforeEach(() => {
    ...
    global.showDeleteConfirm = jest.fn();
        ...
        mockAssigneesSelect.on.mockImplementation((evt, handler) => registerHandler('assigneesSelect', evt, handler));
        mockSelectionEl.on.mockImplementation((evt, selector, handler) => {
             // .on('mousedown', '.remove', handler)
             if (selector) registerHandler('selectionEl', `${evt} ${selector}`, handler);
        ...
        mockJQuery = jest.fn((selector) => {
            ...
            if (selector === document) return {
                on: jest.fn((evt, handler) => registerHandler('document', evt, handler))
            };
  ...
  describe("Delete Confirmation", () => {
    ...
    describe('Select2 Complex Logic', () => {
         ...
         test('sets caret visible on removal mousedown if open', () => {
             ...
             global.triggerEvent('selectionEl', 'mousedown.select2Removal .select2-selection__choice__remove');
