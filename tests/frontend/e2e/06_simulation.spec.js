...
test.describe("Simulation Dashboard Tests", () => {
  ...
  test("SIM-02: Stats cards are visible", async ({ page }) => {
    ...
    await expect(
      page.locator(".card-header", { hasText: "Total Assets" }),
    ).toBeVisible();
    await expect(
      page.locator(".card-header", { hasText: "Maintenance Orders" }),
    ).toBeVisible();
    await expect(
      page.locator(".card-header", { hasText: "Total Users" }),
    ).toBeVisible();
    await expect(
      page.locator(".card-header", { hasText: "Spare Parts" }),
    ).toBeVisible();
  });
  ...
  test("SIM-03: Breakdown simulation section is visible", async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("TRIGGER BREAKDOWN")'),
    ).toBeVisible();
  });
  ...
  test("SIM-04: Bulk Data Generator section is visible", async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("GENERATE DATA")'),
    ).toBeVisible();
  });
});
