...
test.describe("CRUD Functional Tests", () => {
  ...
  test("E2E-C1: test_create_asset", async ({ page }) => {
    ...
    const dataRows = page.locator(
      "#assetsTable table tbody tr:not(:has(.table-empty))",
    );
  ...
  test("E2E-C2: test_edit_asset", async ({ page }) => {
    ...
    const firstIdLink = page
      .locator("#assetsTable table tbody tr")
      .first()
      .locator("a")
      .first();
    ...
    await expect(
      page.locator('.toast-body:has-text("Asset updated successfully")'),
  ...
  test("E2E-C4: test_create_maintenance_order", async ({ page }) => {
    ...
    const dataRows = page.locator(
      "#mosTable table tbody tr:not(:has(.table-empty))",
    );
  ...
  test("E2E-C6: test_delete_mo_from_asset_page", async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    await expect(page.locator("tr", { hasText: moDesc })).toHaveCount(0, {
      timeout: 5000,
    });
  ...
  test("E2E-C10: test_delete_spare_part", async ({ page }) => {
    ...
    await page.fill(
      'input[name="manufacturer_part_id"]',
      "DEL-" + uniqueSuffix,
    );
  ...
  test("E2E-C11: test_edit_user", async ({ page }) => {
    ...
    await expect(dataRows.locator(`text=${newEmail}`)).toBeVisible({
      timeout: 10000,
    });
  });
