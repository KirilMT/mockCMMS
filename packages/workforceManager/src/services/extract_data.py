...
try:
    ...
    # This fallback is for standalone script execution or testing
    from packages.workforceManager.src.config import Config as FlaskConfig
    ...
    try:
        ...
        for col_name, header_text in required_columns.items():
            if col_name == "task_type_col":
                ...
                normalized_search_header = re.sub(r'\\s*/\\s*', '/', normalized_search_header)
                ...
                if matching_columns.empty and col_name not in ["planning_notes_col", "priority_col", "ticket_mo_col"]:
                    ...
                    default_value = ''
                    if col_name == "priority_col": default_value = 'R'
        ...
        for i in range(len(filtered_df)):
            ...
            if not val_scheduler_group_task or val_scheduler_group_task.lower() == 'nan':
                error_messages.append(f"Excel Row {filtered_df.index[i] + 1}: Scheduler Group / Task cannot be blank.")
                continue
            ...
            ticket_url = ""
            if str(row_data.get("task_type", "")).upper() == 'REP' and ticket_mo and ticket_mo.lower() != 'nan':
                try:
        ...
        if not extracted_data and not error_messages:
            error_messages.append(f"No tasks found after filtering sheet '{sheet_name}'.")
...
def extract_data(excel_file_object=None):
    ...
    If DATA_SOURCE is 'api', it fetches from the mockCMMS API.
    If DATA_SOURCE is 'excel', it uses the provided excel_file_object.
    """
...
def get_current_day():
    current_date = _now()
    return (current_date - timedelta(days=1)).strftime("%A") if current_date.hour < 6 else current_date.strftime("%A")
...
def find_and_filter_data(df, current_day, current_shift):
    ...
    matching_columns_for_day = [idx for idx, val in enumerate(day_headers_row) if str(val).strip() == target_header]

    if not matching_columns_for_day:
        raise ValueError(f"No columns found for day header '{target_header}'.")

    target_col = next((idx for idx in matching_columns_for_day if str(shift_headers_row.iloc[idx]).lower().strip() == current_shift), None)

    if target_col is None:
        raise ValueError(f"Column for {current_day} with shift '{current_shift}' not found.")

    df.iloc[:, target_col] = pd.to_numeric(df.iloc[:, target_col], errors='coerce')
    filtered_df = df[df.iloc[:, target_col].notna() & (df.iloc[:, target_col] >= 1)].copy()
    filtered_df = filtered_df[filtered_df.index >= 9]

    if filtered_df.empty:
        raise ValueError(f"No data rows found with quantity >= 1 for the current shift.")
