...
def index():
    ...
    recent_breakdowns = MaintenanceOrder.query.filter(
...
def trigger_breakdown():
    ...
    try:
        ...
        asset = Asset.query.filter_by(status="Operational").order_by(db.func.random()).first()
        ...
        flash(f"💥 breakdown triggered on {asset.asset_code}! Reactive Order #{mo.id} created.", "error")
