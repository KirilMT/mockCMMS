...
const applyFilters = function () {
  ...
  if (
    !hasIncompleteFilter &&
    (hasValidFilter || Object.keys(newFilters).length === 0)
  ) {
    if (window.advTable) {
      window.advTable.filters = newFilters;
      window.advTable.currentPage = 1;
...
const fetchTableConfigs = function (pageName) {
  return fetch(`/api/table-config/${pageName}`).then((response) => {
...
const saveTableConfiguration = function () {
  ...
  if (!configName) {
    if (window.ToastNotification)
      window.ToastNotification.error("Please enter a configuration name");
  ...
  return fetch(`/api/table-config/${window.advTable.pageName}`, {
    ...
    .then((data) => {
      if (data.success) {
        if (window.ToastNotification)
          window.ToastNotification.success("Configuration saved successfully!");
    ...
    .catch((error) => {
      ...
      if (window.ToastNotification)
        window.ToastNotification.error("Error saving configuration");
    });
};
...
const loadSelectedConfiguration = function () {
  ...
  return fetchTableConfigs(window.advTable.pageName)
    ...
    .catch((error) => {
      if (window.ToastNotification)
        window.ToastNotification.error(
          "Error loading saved view: " + error.message,
        );
    });
};
